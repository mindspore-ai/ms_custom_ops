...
以[`ops/ascendc/apply_rotary_pos_emb_ms`](../ops/ascendc/apply_rotary_pos_emb_ms)为例。
...
└── apply_rotary_pos_emb_ms.cc       # 算子在MindSpore中的接入，包括InferShape（与op_host中的InferShape逻辑相同，但是实现接口不一致）、静态图KernelMod接入、动态图等代码
```
