 ...
 */
#include <iostream>
#include "apply_rotary_pos_emb_ms_tiling.h"  // NOLINT(build/include_subdir)
...
namespace optiling {
constexpr uint32_t APPLY_ROTARY_POS_EMB_MS_USE_TBUF_COUNT = 6;
constexpr uint32_t APPLY_ROTARY_POS_EMB_MS_USE_TBUF_COSSIN_COUNT = 2;
constexpr uint32_t APPLY_ROTARY_POS_EMB_MS_TILINGKEY_FP16 = 1;
constexpr uint32_t APPLY_ROTARY_POS_EMB_MS_TILINGKEY_FP32 = 2;
constexpr uint32_t APPLY_ROTARY_POS_EMB_MS_TILINGKEY_FACTOR = 10;
constexpr uint32_t APPLY_ROTARY_POS_EMB_MS_ROTARY_DIM_FACTOR = 2;
...
static ge::graphStatus ApplyRotaryPosEmbMSTiling(gert::TilingContext *context) {
  ApplyRotaryPosEmbMSTilingData tiling;
  ...
  uint32_t rotary_dim = cos_head_dim * APPLY_ROTARY_POS_EMB_MS_ROTARY_DIM_FACTOR;
  ...
  uint32_t ub_use = query_head_dim * (key_head_num + query_head_num) * ge::GetSizeByDataType(query_type) +
                    APPLY_ROTARY_POS_EMB_MS_USE_TBUF_COSSIN_COUNT * cos_head_dim * ge::GetSizeByDataType(cos_type) +
                    (tiling.get_qHiddenSize() + tiling.get_kHiddenSize()) * APPLY_ROTARY_POS_EMB_MS_USE_TBUF_COUNT *
                      ge::GetSizeByDataType(query_type);
  ...
  if (query_type == ge::DataType::DT_FLOAT16) {
    tiling_key = APPLY_ROTARY_POS_EMB_MS_TILINGKEY_FP16;
  } else if (query_type == ge::DataType::DT_FLOAT) {
    tiling_key = APPLY_ROTARY_POS_EMB_MS_TILINGKEY_FP32;
  }
  tiling_key = tiling_key * APPLY_ROTARY_POS_EMB_MS_TILINGKEY_FACTOR + is_split;
...
class ApplyRotaryPosEmbMS : public OpDef {
 public:
  explicit ApplyRotaryPosEmbMS(const char *name) : OpDef(name) {
    ...
    this->SetInferShape(ge::ApplyRotaryPosEmbMSInferShape).SetInferDataType(ge::ApplyRotaryPosEmbMSInferDataType);
    this->AICore().SetTiling(optiling::ApplyRotaryPosEmbMSTiling).AddConfig("ascend310p");
  }
};
