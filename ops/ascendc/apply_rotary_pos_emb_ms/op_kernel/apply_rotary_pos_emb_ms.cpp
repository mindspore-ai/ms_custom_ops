...
class KernelApplyRotaryPosEmbMS {
 public:
  __aicore__ inline KernelApplyRotaryPosEmbMS(ApplyRotaryPosEmbMSTilingData *tiling, AscendC::TPipe *tPipe)
      : tilingData(tiling), pipe(tPipe) {}
 ...
 private:
  ...
  ApplyRotaryPosEmbMSTilingData *tilingData = nullptr;
};

extern "C" __global__ __aicore__ void apply_rotary_pos_emb_ms(GM_ADDR query, GM_ADDR key, GM_ADDR cos, GM_ADDR sin,
  ...
  AscendC::TPipe pipe;
  if (TILING_KEY_IS(20)) {
    KernelApplyRotaryPosEmbMS<float, false> op(&tilingData, &pipe);
    op.Init(query, key, cos, sin, outq, outk, usrWorkspace);
    op.Process();
  } else if (TILING_KEY_IS(10)) {
    KernelApplyRotaryPosEmbMS<half, false> op(&tilingData, &pipe);
    op.Init(query, key, cos, sin, outq, outk, usrWorkspace);
    op.Process();
  } else if (TILING_KEY_IS(21)) {
    KernelApplyRotaryPosEmbMS<float, true> op(&tilingData, &pipe);
    op.Init(query, key, cos, sin, outq, outk, usrWorkspace);
    op.Process();
  } else if (TILING_KEY_IS(11)) {
    KernelApplyRotaryPosEmbMS<half, true> op(&tilingData, &pipe);
    op.Init(query, key, cos, sin, outq, outk, usrWorkspace);
    op.Process();
  }
}
...
void apply_rotary_pos_emb_ms_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *query, uint8_t *key,
                                uint8_t *cos, uint8_t *sin, uint8_t *outq, uint8_t *outk, uint8_t *workspace,
                                uint8_t *tiling) {
  apply_rotary_pos_emb_ms<<<blockDim, l2ctrl, stream>>>(query, key, cos, sin, outq, outk, workspace, tiling);
}
#endif
