# apply_rotary_pos_emb_ms算子

## 描述

apply_rotary_pos_emb_ms算子用于计算旋转编码操作。且支持部分数据参与选择位置编码计算。

## 输入参数

| Name                | DType           | Shape                                  | Optional | Inplace | Format | Description                                            |
|---------------------|-----------------|----------------------------------------|----------|---------|--------|--------------------------------------------------------|
| query               | Tensor(dtype=FP16/FP32)          | 3维[tokens, q_head_num, qk_head_dim] | No       | Yes      | ND     | 执行旋转位置编码的query，同时作为输出。       |
| key                 | Tensor(dtype=FP16/FP32)          | 3维[tokens, k_head_num, qk_head_dim] | No       | Yes      | ND     | 执行旋转位置编码的key，同时作为输出。   |  
...
| layout              | string          | No                                     | No      | No      | string | 表示输入Tensor的布局格式                    |
| rotary_mode         | string          | No                                     | No      | No      | string | 表示支持计算公式中的旋转模式                              |
...
- head_dim: 令`rotary_head_dim = 2 * cos_sin_head_dim`。要求`qk_head_dim >= rotary_head_dim`, qk_head_dim 不能小于rotary_head_dim。当`qk_head_dim > rotary_head_dim`时，只对`query/key[...:rotary_head_dim]` 做旋转位置编码。且`(qk_head_dim - rotary_head_dim)* size(dtype)`必须能被32整除。`cos_sin_head_dim * sizeof(dtype)`必须能被32整除。

## 另见

|算子|特点|
|----|----|
|[apply_rotary_pos_emb_atb](https://gitee.com/mindspore/ms_custom_ops/blob/master/ops/c_api/apply_rotary_pos_emb_atb/apply_rotary_pos_emb_atb.md)<strong style="color:red">【推荐】</strong>|输入参数query/key/cos/sin要求两维, 支持half、quarter模式|
...
ms_custom_ops.apply_rotary_pos_emb_ms(query, key, cos, sin, layout, rotary_mode)
