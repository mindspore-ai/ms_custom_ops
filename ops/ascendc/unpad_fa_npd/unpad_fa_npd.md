...
query = Tensor(np.random.uniform(-1, 1, size=(q_tokens, head_num * head_dim)).astype(np.float16))
key = np.random.uniform(-1, 1, size=(kv_tokens, kv_head_num * head_dim))
value = np.random.uniform(-1, 1, size=(kv_tokens, kv_head_num * head_dim))

q_seq_len = Tensor(q_seq).move_to("CPU")
kv_seq_len = Tensor(kv_seq).move_to("CPU")

# Permute to NPD format
npd_key = key.reshape(kv_tokens // block_size, block_size, kv_head_num, head_dim)
npd_value = value.reshape(kv_tokens // block_size, block_size, kv_head_num, head_dim)
npd_key = npd_key.transpose(0,2,1,3)
...
print(attention_out.shape)  # 期望: (q_tokens, head_num * head_dim)
```
