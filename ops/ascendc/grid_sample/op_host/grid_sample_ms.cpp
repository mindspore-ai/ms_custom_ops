 ...
 */
#include "grid_sample_ms_tiling.h"  // NOLINT(build/include_subdir)
...
namespace ops {
class GridSampleMS : public OpDef {
 public:
  explicit GridSampleMS(const char *name) : OpDef(name) {
    this->Input("input")
      .ParamType(REQUIRED)
    ...
    this->SetInferShape(ge::GridSampleMSInferShape).SetInferDataType(ge::GridSampleMSInferDataType);
    this->AICore().SetTiling(optiling::GridSampleMSTiling).AddConfig("ascend310p");
  }
};
OP_ADD(GridSampleMS);
}  // namespace ops
