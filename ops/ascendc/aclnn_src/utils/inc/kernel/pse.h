...
__aicore__ inline void DataCopyInCommon(LocalTensor<INPUT_T> &dstTensor, GlobalTensor<INPUT_T> &srcTensor,
  ...
  if constexpr (hasPse == true) {
    ...
    dataCopyParams.dstStride = 0;                                       // gap
    if (actualS2Len * 2 % blockBytes == 0) {
      ...
      if constexpr (IsSameType<INPUT_T, float>::value) {
        if (dataCopyPadParams.rightPadding > blockSize) {
          // 8对齐场景，vector需要16对齐，在data copy的时候需要手动补0
...
__aicore__ inline void DataCopyIn(LocalTensor<INPUT_T> &dstTensor, GlobalTensor<INPUT_T> &srcTensor, int64_t offset,
  ...
  if constexpr (hasPse == true) {
    ...
    int32_t alignedS2Size = Align(s2Size);
...
__aicore__ inline int64_t PseComputeOffset(PseInfo &pseInfo) {
  if constexpr (hasPse == true) {
    ...
    if (pseInfo.pseShapeType == pseS1S2) {
      ...
      s1Offset = pseInfo.s1oIdx * pseInfo.s1BaseSize * pseInfo.s2Size +
...
__aicore__ inline int64_t PseAlibiComputeOffset(PseInfo &pseInfo) {
  if constexpr (hasPse == true) {
    ...
    int64_t row = pseInfo.s1oIdx * pseInfo.s1BaseSize + pseInfo.loopIdx * pseInfo.vec1S1BaseSize;
...
__aicore__ inline bool NeedPseAlibiCompute(PseInfo &pseInfo) {
  if constexpr (hasPse == true) {
    // Alibi编码只计算下三角
    if (pseInfo.s1oIdx * pseInfo.s1BaseSize + (pseInfo.loopIdx + 1) * pseInfo.vec1S1BaseSize <=
...
__aicore__ inline void PseAlibiCopyIn(LocalTensor<T> &dstTensor, LocalTensor<INPUT_T> &tmpTensor,
                                      GlobalTensor<INPUT_T> &srcTensor, PseInfo &pseInfo) {
  if constexpr (hasPse == true) {
    ...
    if constexpr (IsSameType<INPUT_T, T>::value) {
      if (!pseInfo.align8) {
        ...
        DataCopyInAlign8<INPUT_T, hasPse>(dstTensor, srcTensor, offset, pseInfo.vec1S1RealSize, pseInfo.readS2Size,
                                          pseInfo.pseS2Size);
      }
      return;
    }
...
__aicore__ inline void PseSlopeCopyIn(LocalTensor<T> &dstTensor, LocalTensor<half> &helpTensor,
  ...
  if constexpr (hasPse == true) {
    ...
    if (pseInfo.needCast) {
      ...
      int64_t s1Offset = pseInfo.s1oIdx * pseInfo.s1BaseSize + pseInfo.loopIdx * pseInfo.vec1S1BaseSize;
      int64_t s2Offset = pseInfo.s2StartIdx + pseInfo.s2LoopCount * pseInfo.s2BaseNratioSize;

      float posShift = static_cast<float>(s2Offset + pseInfo.kvStartIdx - s1Offset - pseInfo.qStartIdx);
      ...
      if (pseInfo.pseType == static_cast<uint32_t>(PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE)) {
...
__aicore__ inline void PseCopyIn(LocalTensor<T> &dstTensor, LocalTensor<INPUT_T> &tmpTensor,
  ...
  if constexpr (hasPse == true) {
    ...
    int64_t s1Size =
      pseInfo.pseShapeType == pse1S2 ? (pseInfo.blockCount == 0 ? 1 : pseInfo.blockCount) : pseInfo.vec1S1RealSize;
...
__aicore__ inline void PseInnerAlibiCreate(GlobalTensor<half> &dstTensor, LocalTensor<half> &helpTensor,
                                           PseInfo &pseInfo) {
  if constexpr (hasPse == true) {
    if (pseInfo.pseType != static_cast<uint32_t>(PseTypeEnum::PSE_INNER_MUL_ADD_TYPE) &&
        pseInfo.pseType != static_cast<uint32_t>(PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE)) {
      return;
    }
