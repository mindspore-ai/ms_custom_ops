...
# ms_custom_ops modification
CURRENT_DIR=$(dirname "$(readlink -f ${BASH_SOURCE[0]})")
BUILD_BASE_DIR=${CURRENT_DIR}/../../../build/ascendc
...
CLANG="false"
VERBOSE="false"
FORCE_CLEAN="false"  # 增量编译支持: 默认不清理，使用 --clean 参数强制清理
...
function help_info() {
    ...
    echo
    echo "--clean              Force clean build directory before compilation. By default, incremental build is used."
    echo
}
...
function clean()
{
    if [ "${FORCE_CLEAN}" == "true" ]; then
        log "Info: Force clean enabled, removing build directory..."
        if [ -n "${BUILD_DIR}" ];then
            rm -rf ${BUILD_DIR}
        fi
    else
        log "Info: Incremental build enabled, keeping existing build artifacts..."
    fi
...
if [ "${BUILD}" == "host" ];then
    ...
    build_host
    # TO DO
    rm -rf ${BUILD_BASE_DIR}/output
    mkdir -p ${BUILD_BASE_DIR}/output
    cp ${BUILD_DIR}/*.run ${BUILD_BASE_DIR}/output
