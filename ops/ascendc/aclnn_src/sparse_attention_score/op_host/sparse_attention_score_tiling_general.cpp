...
/*!
 * \file sparse_attention_score_tiling_general.cpp
 * \brief
 */
...
#include "sparse_attention_score_tiling_common.h"

/// \brief (Quick-and-dirty hack) Local variable to store verbosity level
static std::uint8_t aclnn_sparse_attention_score_tiling_verbosity = 0;
/// \brief (Quick-and-dirty hack) Verbosity level threshold for SparseAttentionScoreTilingBase::SetCoreParams()
static constexpr std::uint8_t sparse_attention_score_tiling_base_set_core_params_print = 2;

/// \brief (Quick-and-dirty hack) Set the local verbosity level
extern "C" [[gnu::visibility("default")]]
void aclnn_set_sparse_attention_score_tiling_verbosity(std::uint8_t v) {
  if (aclnn_sparse_attention_score_tiling_verbosity || v) {
    std::fprintf(stderr, "%s(): previous verbosity level was %u, new level will be %u\n", __func__,
                 aclnn_sparse_attention_score_tiling_verbosity, v);
  }
  aclnn_sparse_attention_score_tiling_verbosity = v;
}

/// \brief (Quick-and-dirty hack) Local variable to store the sparse block height
static std::uint32_t aclnn_sparse_attention_score_block_height = 128;
/// \brief (Quick-and-dirty hack) Local variable to store the sparse block width
static std::uint32_t aclnn_sparse_attention_score_block_width = 256;

extern "C" [[gnu::visibility("default")]]
void aclnn_set_sparse_attention_score_tiling_block_shape(std::uint32_t height, std::uint32_t width) {
  if (aclnn_sparse_attention_score_tiling_verbosity >= sparse_attention_score_tiling_base_set_core_params_print) {
    std::fprintf(stderr, "%s(): previous block shape was %ux%u, new shape will be %ux%u\n", __func__,
                 aclnn_sparse_attention_score_block_height, aclnn_sparse_attention_score_block_width, height, width);
  }

  aclnn_sparse_attention_score_block_height = height;
  aclnn_sparse_attention_score_block_width = width;
}
...
const size_t SELECT_BLOCK_IDX_INPUT_INDEX = 12UL;
...
/**
 * @description: 将num1向上对齐至num2的倍数
 * @param { T} num2
 * @return {*}
 * @Date: 2025-06-10 14:46:14
...
/**
 * @description: 将num1向下对齐至num2的倍数
 * @param { T} num2
 * @return {*}
 * @Date: 2025-06-10 14:49:17
 * @author: c00558882 chenjingheng
 */
template <typename T>
...
/**
 * @description: 将num1平均分成num2份，结果向上取整
 * @param { T} num2
 * @return {*}
 * @Date: 2025-06-10 14:49:42
 * @author: c00558882 chenjingheng
 */
template <typename T>
...
/**
 * @description: 将n1分段，每段长n2, 最后一共多少段(最后一段<=n2)
 * @return {*}
 * @Date: 2025-06-10 14:51:44
 * @author: c00558882 chenjingheng
 */
template <typename T>
...
/**
 * @description: 将num1分段，每段长n2, 返回尾段长度
 * @param { T} num2
 * @return {*}
 * @Date: 2025-06-10 14:54:25
 * @author: c00558882 chenjingheng
 */
template <typename T>
...
class TilingKey {
 public:
  ...
  uint32_t reserved : 23;  // to fulfil 32 bit, if add new template bit then decrease this number
};
...
class SparseAttentionScoreTilingBase : public TilingBaseClass {
 public:
  explicit SparseAttentionScoreTilingBase(gert::TilingContext *context) : TilingBaseClass(context) { Reset(); }
  ~SparseAttentionScoreTilingBase() override = default;
 ...
 protected:
  ...
  bool isHighPrecision;  // fp16 high precision mode
  ...
  uint32_t selectBlockLen;
  uint32_t selectBlockNum;
  ...
  SparseAttentionScoreGeneralTilingData tilingData;
};

int64_t SparseAttentionScoreTilingBase::GetNRatio() { return BMM_SOFTMAX_RATIO; }

ge::graphStatus SparseAttentionScoreTilingBase::GetPlatformInfo() {
  auto platformInfoPtr = context_->GetPlatformInfo();
  if (platformInfoPtr == nullptr) {
    auto compileInfoPtr = reinterpret_cast<const SparseAttentionScoreCompileInfo *>(context_->GetCompileInfo());
...
ge::graphStatus SparseAttentionScoreTilingBase::CheckContext() {
...
void SparseAttentionScoreTilingBase::SetSparseTilingInfo(SparseEnum &sparseType) {
...
void SparseAttentionScoreTilingBase::EnableBandInvalidLineImplMode() {
...
/**
 * @description: 根据sparse模式，更改preToken与nextToken
 * @param {SparseEnum} &sparseType
 * @return {*}
 * @Date: 2025-06-10 15:12:34
 * @author: c00558882 chenjingheng
 */
bool SparseAttentionScoreTilingBase::PretokenAndNexttokenAdjustment(SparseEnum &sparseType) {
  ...
  } else if (sparseMode == BAND) {  /// 因为band模式时，输入的pre和next是以右下角为参考点，所以要转换成左上角作为参考点
...
bool SparseAttentionScoreTilingBase::SparseBandModeCheck(int64_t maxS1Value, int64_t maxS2Value, int64_t minS1Value,
  ...
  if (preTokens < 0 && nextTokens < 0) {
    OPS_LOG_E(context_, "PreTokens[%ld] and nextTokens[%ld] config error, there is no valid data block.", oriPreTokens,
              oriNextTokens);
    return false;
  }
  ...
  if (preTokens < 0 && nextTokens >= 0) {
    ...
    if (nextTokens >= absPreTokens && absPreTokens < minS2Value) {
      ...
      OPS_LOG_E(context_, "PreTokens[%ld] and nextTokens[%ld] config error with S1[%ld], there is no valid data block.",
  ...
  if (preTokens >= 0 && nextTokens < 0) {
    ...
    if (absNextTokens <= preTokens && absNextTokens < minS1Value) {
      ...
      OPS_LOG_E(context_, "PreTokens[%ld] and nextTokens[%ld] config error with S2[%ld], there is no valid data block.",
...
bool SparseAttentionScoreTilingBase::SparseModeProcess(SparseEnum &sparseType) {
...
bool SparseAttentionScoreTilingBase::GetSparseInfo(SparseEnum &sparseType) {
...
bool SparseAttentionScoreTilingBase::SetPseAlibiParams() {
  auto &inputParams = tilingData.inputParams;
...
ge::graphStatus SparseAttentionScoreTilingBase::GetShapeAttrsInfo() {
  ...
  inputParams.set_selectBlockLen(static_cast<uint32_t>(selectBlockLen));
  inputParams.set_selectBlockNum(static_cast<uint32_t>(selectBlockNum));
  OPS_LOG_D(context_,
            "input params: bn2gs1s2d[%ld, %ld, %ld, %ld, %ld, %ld], keepProb[%f], scaleValue[%f],"
            "pseType:%ld, selectBlockLen:%u, selectBlockNum:%u.",
            bSize, n2Size, gSize, s1Size, s2Size, dSize, keepProb, scaleValue, pseType, selectBlockLen, selectBlockNum);

  return ge::GRAPH_SUCCESS;
}

void SparseAttentionScoreTilingBase::Reset() {
...
bool SparseAttentionScoreTilingBase::AnalyzeDtype() {
  inputDtype = context_->GetInputDesc(0)->GetDataType();
  inputDtypeBytes = ge::GetSizeByDataType(inputDtype);
  switch (inputDtype) {
    case ge::DT_FLOAT16:
      bmmDtype = matmul_tiling::DataType::DT_FLOAT16;
      bmm1OutDtype = isHighPrecision ? matmul_tiling::DataType::DT_FLOAT : matmul_tiling::DataType::DT_FLOAT16;
      tilingKeyDType = isHighPrecision ? DtypeEnum::FLOAT16_PRECISION : DtypeEnum::FLOAT16;
      calcTypeSize = isHighPrecision ? ge::GetSizeByDataType(ge::DT_FLOAT) : ge::GetSizeByDataType(inputDtype);
      break;
    case ge::DT_FLOAT:
      bmmDtype = matmul_tiling::DataType::DT_FLOAT;
      bmm1OutDtype = matmul_tiling::DataType::DT_FLOAT;
  ...
  OPS_LOG_D(context_, "Get high precision flag: %d.", isHighPrecision);
  return true;
}

bool SparseAttentionScoreTilingBase::AnalyzeAttrs() {
  auto attrs = context_->GetAttrs();
  size_t idx = 0;
  auto scaleValuePtr = attrs->GetAttrPointer<float>(idx++);
  auto keepProbPtr = attrs->GetAttrPointer<float>(idx++);
  ...
  if (attrs->GetAttrNum() > idx) {
    ...
    isHighPrecision = true;  // use default value
  }
  ...
  if (attrs->GetAttrNum() > idx) {
    auto selectBlockLenPtr = attrs->GetAttrPointer<uint32_t>(idx++);
    selectBlockLen = *selectBlockLenPtr;
    if (selectBlockLen > 0) {  /// 只有选择块的长度大于0，才可以认为是进入了select稀疏模式
      const gert::StorageShape *selectBlockIdxShape =
        context_->GetOptionalInputShape(SELECT_BLOCK_IDX_INPUT_INDEX);  // nsa: [B, N2, S1, W]，其中W就是selectBlockNum
      OPS_ERR_IF(selectBlockIdxShape == nullptr || selectBlockIdxShape->GetStorageShape().GetDimNum() == 0,
                 OPS_REPORT_VECTOR_INNER_ERR(context_, "selectBlockIdxShape is nullptr or not data."),
                 return ge::GRAPH_FAILED);
      uint32_t selectBlockNumIndex = selectBlockIdxShape->GetStorageShape().GetDimNum() - 1;  /// 最后一维的index
      selectBlockNum = selectBlockIdxShape->GetStorageShape().GetDim(selectBlockNumIndex);  /// 获取W就是selectBlockNum
      // printf("========== selectBlockIndex shape: ");
      // for(uint32_t i=0; i<=selectBlockNumIndex;i++){
      //     printf("%u, ", selectBlockIdxShape->GetStorageShape().GetDim(i));
      // }
      // printf("\n");
    }
  }
  ...
  OPS_LOG_D(context_,
            ...
            "input_layout[%s] inner_precise[%d] sparse_mode[%ld] pseType[%ld] selectBlockLen[%u] selectBlockNum[%u].",
            scaleValue, keepProb, preTokens, nextTokens, n1Size, inputLayout, implMode, sparseMode, pseType,
            selectBlockLen, selectBlockNum);
  return true;
}

bool SparseAttentionScoreTilingBase::AnalyzeLayout() {
  ...
  OPS_ERR_IF(!Analyze3DimLayout(queryShape, keyShape, layoutLen) || !Analyze4DimLayout(queryShape, keyShape, layoutLen),
...
void SparseAttentionScoreTilingBase::GetActualSeqLenData(int64_t inputIdx,
                                                         std::array<int64_t, MAX_VAR_LEN_SEQ_LEN> &res,
  ...
  if (actualSeqLenShape.GetDimNum() != 1) {
    OPS_LOG_W(context_, "[%s]actualSeqLenShape is invalid %ld %ld", templateName, actualSeqLenShape.GetDimNum(),
...
bool SparseAttentionScoreTilingBase::Analyze3DimLayout(const gert::Shape &queryShape, const gert::Shape &keyShape,
                                                       size_t layoutLen) {
  if (layoutLen == 3UL) {
    int64_t h1 = 0;
    int64_t h2 = 0;
    if (inputLayout[0] == 'B' && inputLayout[1] == 'S' && inputLayout[2] == 'H') {  // 2: H idx
      ...
      OPS_ERR_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(
          opName, "Query T(%ld) and key T(%ld) need equal to respectively sum of seqLen(%ld) and sekvLen(%ld).", t1Size,
          t2Size, accumS1, accumS2),
        return false);
      for (int64_t i = 0; i < bSize; ++i) {
        OPS_ERR_IF(actualSeqLenData[i] != 0 && actualSeqLenKvData[i] == 0,
                   OPS_REPORT_VECTOR_INNER_ERR(
                     opName, "Batch(%ld) seqLen is not 0 and sekvLen is 0, this scene is not supported.", i),
                   return false);
      }
...
bool SparseAttentionScoreTilingBase::Analyze4DimLayout(const gert::Shape &queryShape, const gert::Shape &keyShape,
                                                       size_t layoutLen) {
  if (layoutLen == 4UL) {
    // 2: N idx, 3: D idx
...
SparseEnum SparseAttentionScoreTilingBase::GetPrefixNList(std::ostringstream &failReason) {
  ...
  if (prefixShape.GetDimNum() != 1) {
    OPS_LOG_W(context_, "[%s] prefixN shape is invalid, DimNum should be 1, but it is %ld.", templateName,
  ...
  for (int64_t i = 0; i < bSize; ++i) {
    if (prefixNData[i] < nMin || prefixNData[i] > s2Size) {
      OPS_LOG_W(context_, "[%s] batch[%ld] prefixN=%ld is invalid, should be in range of [%ld, %ld]", templateName, i,
                prefixNData[i], nMin, s2Size);
      failReason << "batch[" << i << "] prefixN=" << prefixNData[i] << " is invalid, should be in range of [" << nMin
                 << ", " << s2Size << "]";
      return SparseEnum::ALL;
    }
...
void SparseAttentionScoreTilingBase::SetQKVStartIdx() {
...
bool SparseAttentionScoreTilingBase::AnalyzeOptionalInput() {
  ...
  if (attenMaskInput != nullptr && attenMaskShape != nullptr && attenMaskShape->GetStorageShape().GetDimNum() != 0) {
    ...
    if (attenMaskDimNum == ATTENTION_MASK_DIM_NUM_4) {
      ...
      if (attenMaskDim0Size == 1 && attenMaskDim1Size == 1 && attenMaskDim2Size == s1Size &&
        ...
        OPS_LOG_E(context_, "get unsupported atten_mask shape, the shape is [%ld, %ld, %ld, %ld]", attenMaskDim0Size,
                  attenMaskDim1Size, attenMaskDim2Size, attenMaskDim3Size);
        return false;
      }
    } else if (attenMaskDimNum == ATTENTION_MASK_DIM_NUM_2) {
      int64_t attenMaskDim0Size = attenMaskStorageShape.GetDim(0);
      int64_t attenMaskDim1Size = attenMaskStorageShape.GetDim(1);
      if ((attenMaskDim0Size == s1Size && attenMaskDim1Size == s2Size) || (attenMaskCompressMode != NO_COMPRESS_MODE)) {
  ...
  if (dropMaskInput != nullptr && dropMaskShape != nullptr && dropMaskShape->GetStorageShape().GetDimNum() != 0) {
    ...
    if (dropMaskShapeSize < shapeSize) {
      OPS_LOG_E(context_, "Input dropMask shapeSize is invalid, it should not be less than %ld, but got %ld", shapeSize,
                dropMaskShapeSize);
      return false;
    }
    dropMaskExistFlag = 1;
  ...
  // if s2Size aligned to 8, then no need dropMaskOp to transfer dropMask from bit to byte format
...
ge::graphStatus SparseAttentionScoreTilingBase::DoOpTiling() {
  auto &inputParams = tilingData.inputParams;
...
bool SparseAttentionScoreTilingBase::MatchTemplate() {
  ...
  if (s2BasicBlock == std::numeric_limits<int64_t>::max()) {
    OPS_LOG_D(context_,
              ...
              ge::TypeUtils::DataTypeToSerialString(inputDtype).c_str(), isHighPrecision);
    return false;
  }
...
void SparseAttentionScoreTilingBase::CalcS1S2BasicBlock(const BufferNum &bufferNum) {
  ...
  for (int64_t tmpS1BasicBlock = std::min(GetMinS1BasicBlock(), maxS1BasicBlock); tmpS1BasicBlock <= maxS1BasicBlock;
    ...
    OPS_LOG_D(context_, "[%s]get candidate basic block: [%ld, %ld]", templateName, tmpS1BasicBlock, tmpS2BasicBlock);
    if (s2BasicBlock == std::numeric_limits<int64_t>::max()) {
...
void SparseAttentionScoreTilingBase::CalcDBasicBlock() { return; }

int64_t SparseAttentionScoreTilingBase::CalcMaxS1BasicBlockSize(int64_t actualD, const BufferNum &bufferNum) const {
  ...
  int64_t maxS1BasicBlock =
    ...
    (FRACTAL_NUM * bufferNum.bufferS1S2Num + actualD * bufferNum.bufferS1DNum +
     (bufferNum.bufferExpNum + 2) * alignUnit);  // here 2 means SparseSoftMax sum and max output
  return AlignDown(maxS1BasicBlock, FRACTAL_NUM);
}

int64_t SparseAttentionScoreTilingBase::CalcMaxS2BasicBlockSize(const BufferNum &bufferNum,
  ...
  if (expectTemplate.splitD == 0) {
    // here 2 means SparseSoftMax sum and max output
...
bool SparseAttentionScoreTilingBase::IsBasicBlockInSoftMax(const ge::Shape &shape) const {
  // 2 axes at least
...
void SparseAttentionScoreTilingBase::SetCoreParams() {
  auto &coreParams = tilingData.coreParams;
  if (aclnn_sparse_attention_score_tiling_verbosity >= sparse_attention_score_tiling_base_set_core_params_print) {
    fprintf(stderr, "%s: current verbosity level is %u\n", __func__, aclnn_sparse_attention_score_tiling_verbosity);
    printf("=======================  SetCoreParams  ========================\n");
    printf("===== (s1BasicBlock, s2BasicBlock): (%lld, %lld)\n", s1BasicBlock, s2BasicBlock);
  }
...
void SparseAttentionScoreTilingBase::SetMultiBatchCoreParams() {
  auto &coreParams = tilingData.coreParams;
...
void SparseAttentionScoreTilingBase::SetMultiCoreParams() {
...
ge::graphStatus SparseAttentionScoreTilingBase::DoLibApiTiling() {
...
ge::graphStatus SparseAttentionScoreTilingBase::PostTiling() {
  context_->GetRawTilingData()->SetDataSize(tilingData.GetDataSize());  // already check capacity in CheckContext
  ...
  OPS_LOG_D(context_, "[%s] final workspace size:%zu, pseAlibiBaseS1:%ld, pseAlibiBaseS2:%ld.", templateName,
...
bool SparseAttentionScoreTilingBase::SetBmm1TilingInput(int64_t tmpS1BasicBlock, int64_t tmpS2BasicBlock, int64_t batch,
                                                        matmul_tiling::MatmulApiTiling &bmm1) {
...
bool SparseAttentionScoreTilingBase::SetMatMulTiling(int64_t tmpS1BasicBlock, int64_t tmpS2BasicBlock,
...
void SparseAttentionScoreTilingBase::SetSoftMaxTiling() {
  ...
  AscendC::SoftMaxFlashV2TilingFunc(softmaxShape, calcTypeSize, sizeof(float), apiMaxUBSize,
                                    tilingData.softmaxSparseTilingData, true, IsBasicBlockInSoftMax(softmaxShape));
}

void SparseAttentionScoreTilingBase::SetDataCopyTransposeTiling() {
  auto &coreParams = tilingData.coreParams;
...
void SparseAttentionScoreTilingBase::SetTensorSizeParams() {
  ...
  if (inputDtype == ge::DT_BF16 || isHighPrecision) {
    if (expectTemplate.splitS2 == 1) {
...
bool SparseAttentionScoreTilingBase::InitSparseValidArray(std::vector<int64_t> &sparseValidArray, int64_t bIdx) {
  ...
  if (sparseType == static_cast<uint8_t>(SparseEnum::PREFIX)) {
    for (int64_t i = 0; i < static_cast<int64_t>(sparseValidArray.size()); i++) {
      int64_t s2IgnoredEndLen = tilingData.inputParams.get_s1Size() - tilingData.coreParams.get_s1BaseSize() * (i + 1);
      int64_t s2EndLen = 0;
    ...
    for (int64_t i = 0; i < static_cast<int64_t>(sparseValidArray.size()); i++) {
      ...
      int64_t addBlk = std::min(s2BlkNum - validS2Size,
...
bool SparseAttentionScoreTilingBase::PartitionSparseData(const std::vector<int64_t> &sparseRollingArray,
...
void SparseAttentionScoreTilingBase::SetPrefixSparseStartIdx(const std::vector<std::vector<int64_t>> &sparseValidArray,
                                                             MultiCoreParams &multiCoreParams) {
...
bool SparseAttentionScoreTilingBase::SetSparseStartIdx(const std::vector<int64_t> &sparseValidArray,
                                                       MultiCoreParams &multiCoreParams) {
  ...
  if (CheckLogLevel(OP, DLOG_DEBUG) == 1) {
    PrintSparseMaxMinLoadPerCore(sparseValidArray, sparseStartIdx, validAivNum, CeilDivision(loadTotal, validAivNum));
  }
  return true;
}

void SparseAttentionScoreTilingBase::PrintSparseMaxMinLoadPerCore(const std::vector<int64_t> &sparseValidArray,
  ...
  for (int64_t idx = 0; idx < validAivNum; ++idx) {
    ...
    if (s1OuterEndIdx > s1OuterStartIdx) {
      loadSize =
        std::accumulate(sparseValidArray.begin() + s1OuterStartIdx, sparseValidArray.begin() + s1OuterEndIdx, 0);
    } else {
...
void SparseAttentionScoreTilingBase::SetSparseParams() {
...
ge::graphStatus SparseAttentionScoreTilingBase::GetWorkspaceSize() {
  auto &tensorSizeParams = tilingData.tensorSizeParams;
  auto &coreParams = tilingData.coreParams;
...
class SparseAttentionScoreTilingS1Bn2gs1 : public SparseAttentionScoreTilingBase {
 public:
  explicit SparseAttentionScoreTilingS1Bn2gs1(gert::TilingContext *context) : SparseAttentionScoreTilingBase(context) {
    expectTemplate.splitS1 = 1;
    expectTemplate.splitD = 1;
    templateName = "SparseAttentionScoreS1Bn2gs1";
  }
  ~SparseAttentionScoreTilingS1Bn2gs1() override = default;
 ...
 protected:
  ...
  bool AnalyzeDtype() override {
    OPS_ERR_IF(!SparseAttentionScoreTilingBase::AnalyzeDtype(),
  ...
  void SetEnableL1Reuse() {
    ...
    if ((s2Size >= L1REUSE_S2_LIMIT_512 && bSize * n2Size * gSize > L1REUSE_BNG_LIMIT_64 && dSize <= L1REUSE_D_Limit) ||
  ...
  void SetCoreParams() override {
    SetEnableL1Reuse();
    // 稀疏场景不开启S1轴N:1配比
    SparseAttentionScoreTilingBase::SetCoreParams();
    if (tilingData.inputParams.get_sparseType() != static_cast<uint8_t>(SparseEnum::ALL)) {
      return;
    }
    SetMultiCoreParams();
  ...
  bool IsTemplateMatched() const override {
    ...
    if (static_cast<uint64_t>(n2Size * gSize * ((alignedS1 + alignedS2) * dSize + alignedS2) * inputDtypeBytes) >=
          aicoreParams_.l1Size ||
        static_cast<uint64_t>(n2Size * gSize * (alignedS1 + dSize) * alignedS2 * inputDtypeBytes) >=
          aicoreParams_.l1Size) {
      return true;
    }
  ...
  uint64_t GetTilingKey() const override {
    return GET_TILINGKEY(AxisEnum::S1, AxisEnum::D, AxisEnum::NONE, implMode, tilingKeyDType, tilingKeyLayout,
                         ...
                         PerformanceOrientedEnum::BIG_DOUBLE_BUFFER, hasDropOut, hasAttenMask, hasPse, enableL1Reuse);
  }
  ...
  ge::graphStatus GetWorkspaceSize() override {
    if (tilingData.inputParams.get_sparseType() == 0) {
      ...
      tilingData.multiCoreParams.set_coreNum(std::min(static_cast<int32_t>(aivNum), actualUsedAivNum));
    }
...
class SparseAttentionScoreTilingB : public SparseAttentionScoreTilingBase {
 public:
  explicit SparseAttentionScoreTilingB(gert::TilingContext *context) : SparseAttentionScoreTilingBase(context) {
    templateName = "SparseAttentionScoreB";
  }
  ~SparseAttentionScoreTilingB() override = default;
 ...
 protected:
  ...
  void SetMultiCoreParams() override {
    ...
    multiCoreParams.set_coreNum(static_cast<int32_t>(CeilDivision(totalSize, multiCoreParams.get_splitFactorSize())));
    OPS_LOG_D(context_,
  ...
  void SetTensorSizeParams() override {
    ...
    tensorSizeParams.set_dropMaskUbSize(dropMaskExistFlag * s1BasicBlock * AlignUp(s2BasicBlock, DROP_MASK_ALIGN_UNIT) /
                                        BYTE_BIT_NUM / inputDtypeBytes);
    if (tensorSizeParams.get_pseUbSize() > 0) {
      hasPse = true;
    }
    if (tensorSizeParams.get_dropMaskUbSize() > 0) {
...
class SparseAttentionScoreTilingS1s2Bn2gs1 : public SparseAttentionScoreTilingBase {
 public:
  explicit SparseAttentionScoreTilingS1s2Bn2gs1(gert::TilingContext *context)
      : SparseAttentionScoreTilingBase(context) {
    expectTemplate.splitS1 = 1;
    expectTemplate.splitS2 = 1;
    templateName = "SparseAttentionScoreS1s2Bn2gs1";
  }
  ~SparseAttentionScoreTilingS1s2Bn2gs1() override = default;
 ...
 protected:
  ...
  void CalcS1S2BasicBlock(const BufferNum &bufferNum) override {
    ...
    /// s1与s2方向上的basicBlock进行固定, by_cjh
    // s1BasicBlock = 64L;
    // s2BasicBlock = 128L;
    if (aclnn_sparse_attention_score_block_height) {
      s1BasicBlock = static_cast<decltype(s1BasicBlock)>(aclnn_sparse_attention_score_block_height);
    }
    if (aclnn_sparse_attention_score_block_width) {
      s2BasicBlock = static_cast<decltype(s2BasicBlock)>(aclnn_sparse_attention_score_block_width);
    }
  ...
  uint64_t GetTilingKey() const override {
    // printf("===== Tiling key: %lu: \n", GET_TILINGKEY(AxisEnum::S1, AxisEnum::S2, AxisEnum::NONE, implMode,
    // tilingKeyDType, tilingKeyLayout,
    ...
    return GET_TILINGKEY(AxisEnum::S1, AxisEnum::S2, AxisEnum::NONE, implMode, tilingKeyDType, tilingKeyLayout,
                         tilingKeyBmm1Format, SparseEnum::ANY, PerformanceOrientedEnum::BIG_DOUBLE_BUFFER, hasDropOut,
                         hasAttenMask, hasPse, enableL1Reuse);
  }
  ...
  void RefreshSplitFactor() {
    ...
    if (enableL1Reuse) {
      ...
      multiCoreParams.set_splitFactorSize(CeilDivision(totalSize, static_cast<int64_t>(multiCoreParams.get_coreNum())) *
                                          AICAIV_RATIO_2);
      multiCoreParams.set_splitFactorTailSize(CalcTailSize(totalSize, multiCoreParams.get_splitFactorSize()));
    }
  }
  ...
  ge::graphStatus GetWorkspaceSize() override {
    ...
    if (dSize <= D_SPECIFIC_SIZE) {
      ...
      workspaces[0] =
        static_cast<size_t>(
          (bmm1Bytes * SPACE_NUM_2 + SPACE_NUM_2 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) * aivNum) +
        WORK_SPACE_RESERVE_SIZE;
      // NZND场景，stage1占用3倍的空间，stage2占用2倍空间
      if (s2Size % S2_NZTOND_SIZE_64 != 0) {
        workspaces[0] =
          static_cast<size_t>(
            (bmm1Bytes * SPACE_NUM_3 + SPACE_NUM_2 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) * aivNum) +
          WORK_SPACE_RESERVE_SIZE;
      }
      ...
      if (inputDtypeBytes == DATA_TYPE_FP32) {
        workspaces[0] =
          static_cast<size_t>(
            (bmm1Bytes * SPACE_NUM_4 + SPACE_NUM_2 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) * aivNum) +
          WORK_SPACE_RESERVE_SIZE;
      }
    } else {
      ...
      workspaces[0] =
        static_cast<size_t>(
          (bmm1Bytes * SPACE_NUM_2 + SPACE_NUM_4 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) * aivNum) +
        WORK_SPACE_RESERVE_SIZE;
      // NZND场景，stage1占用3倍的空间，stage2占用4倍空间
      if (s2Size % S2_NZTOND_SIZE_64 != 0) {
        workspaces[0] =
          static_cast<size_t>(
            (bmm1Bytes * SPACE_NUM_3 + SPACE_NUM_4 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) * aivNum) +
          WORK_SPACE_RESERVE_SIZE;
      }
      ...
      if (inputDtypeBytes == DATA_TYPE_FP32) {
        workspaces[0] =
          static_cast<size_t>(
            (bmm1Bytes * SPACE_NUM_4 + SPACE_NUM_4 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) * aivNum) +
          WORK_SPACE_RESERVE_SIZE;
      }
    }
    if (pseType == PSE_INNER_MUL_ADD_TYPE || pseType == PSE_INNER_MUL_ADD_SQRT_TYPE) {
...
class SparseAttentionVarLenScoreTiling : public SparseAttentionScoreTilingBase {
 public:
  explicit SparseAttentionVarLenScoreTiling(gert::TilingContext *context) : SparseAttentionScoreTilingBase(context) {
    expectTemplate.splitS1 = 1;
    expectTemplate.splitS2 = 1;
    templateName = "SparseAttentionVarLenScore";
  }
  ~SparseAttentionVarLenScoreTiling() override = default;

 protected:
  int64_t s2sizeLimitMax = 1024;

  int64_t GetNRatio() override { return 8L; }
  ...
  uint64_t GetTilingKey() const override {
    // not care about layout in tiling key, pass BSND(enum value is 0)
    return GET_TILINGKEY(AxisEnum::S1, AxisEnum::S2, AxisEnum::NONE, implMode, tilingKeyDType, tilingKeyLayout,
                         SparseEnum::ANY, PerformanceOrientedEnum::BIG_DOUBLE_BUFFER, hasDropOut, hasAttenMask, hasPse);
  }
  ...
  bool SetPseAlibiParams() override {
    ...
    if (pseType == PSE_INNER_MUL_ADD_TYPE || pseType == PSE_INNER_MUL_ADD_SQRT_TYPE) {
      OPS_ERR_IF(tilingData.inputParams.get_sparseType() == static_cast<uint8_t>(SparseEnum::RIGHT_DOWN_CAUSAL_BAND),
                 OPS_REPORT_VECTOR_INNER_ERR(opName, "INNER Pse alibi only support BAND_LEFT_UP_CAUSAL sparse type."),
                 return false);
  ...
  ge::graphStatus GetWorkspaceSize() override {
    ...
    if (dSize <= D_SPECIFIC_SIZE) {
      ...
      workspaces[0] = static_cast<size_t>((bmm1Byetes * SPACE_NUM_3 + stage1Bytes * SPACE_NUM_2 +
                                           SPACE_NUM_2 * coreParams.get_s1BaseSize() * alignedD * calcTypeSize) *
                                          aivNum) +
                      WORK_SPACE_RESERVE_SIZE;
    } else {
  ...
  bool CheckPretokenAndNexttoken(SparseEnum &sparseType) {
    if (sparseMode == ALL_MASK) {
      ...
      for (int64_t i = 0L; i < bSize; ++i) {
        if (actualSeqLenData[i] > actualSeqLenKvData[i]) {
          OPS_LOG_E(context_, "Batch[%ld] s1[%ld] is larger than s2[%ld], exist invalid row.", i, actualSeqLenData[i],
      ...
      if (preTokens < firstS2) {
        OPS_LOG_E(context_, "Band_LeftUpCausal mode: pre_tokens[%ld] is smaller than first valid s2[%ld].", preTokens,
                  firstS2);
        return false;
      }
  ...
  bool SparseNoMaskModeCheck(int64_t maxS1Value, int64_t maxS2Value, int64_t minS2Value, SparseEnum &sparseType) {
    ...
    if (preTokens < 0) {
      ...
      if (nextTokens >= absPreTokens) {
        ...
        OPS_LOG_E(context_, "preTokens[%ld] and nextTokens[%ld] config error with S[%ld], has invalid data block.",
    ...
    for (int64_t i = 0; i < bSize; ++i) {
      if (actualSeqLenData[i] > actualSeqLenKvData[i]) {
        if (prefixNData[i] < 0 || prefixNData[i] > actualSeqLenKvData[i]) {
          OPS_LOG_E(context_, "[%s] batch[%ld] prefixN=%ld is invalid, should be in range of [0, %ld]", templateName, i,
        ...
        if (prefixNData[i] < actualSeqLenKvData[i] - actualSeqLenData[i] || prefixNData[i] > actualSeqLenKvData[i]) {
          OPS_LOG_E(context_, "[%s] batch[%ld] prefixN=%ld is invalid, should be in range of [%ld, %ld]", templateName,
                    i, prefixNData[i], actualSeqLenKvData[i] - actualSeqLenData[i], actualSeqLenKvData[i]);
          return false;
        }
      }
    }
  ...
  bool GetSparseInfo(SparseEnum &sparseType) override {
    OPS_LOG_D(context_, "check sparse feature: preTokens[%ld], nextTokens[%ld], s1[%ld], s2[%ld], attenMaskExist[%d].",
              preTokens, nextTokens, s1Size, s2Size, attenMaskExistFlag);
  ...
  bool SetSparseStartIdx(const std::vector<int64_t> &sparseValidArray, MultiCoreParams &multiCoreParams) override {
    ...
    OPS_ERR_IF(totalSize <= 0, OPS_REPORT_VECTOR_INNER_ERR(opName, "totalSize should be larger than 0."), return false);
    ...
    for (int64_t idx = 1; idx < MAX_AIV_NUM; ++idx) {
      ...
      if ((start + 1) < tmpsparseStartIdx[idx]) {
        ...
        singleLoadValue =
          ((singleLoadValue - avgVal) > (avgVal - redoSingleLoadValue)) ? redoSingleLoadValue : singleLoadValue;
...
class SparseAttentionScoreTilingDropMask : public SparseAttentionScoreTilingBase {
 public:
  explicit SparseAttentionScoreTilingDropMask(gert::TilingContext *context) : SparseAttentionScoreTilingBase(context) {
    if (context_->GetRawTilingData()->GetCapacity() >= tilingData.GetDataSize()) {
      OPS_ERR_IF(
        memset_s(context_->GetRawTilingData()->GetData(), tilingData.GetDataSize(), 0, tilingData.GetDataSize()) != EOK,
        OPS_REPORT_VECTOR_INNER_ERR(opName, "fail to memset tiling data"), return;);
    }
  }
  ~SparseAttentionScoreTilingDropMask() override = default;
 ...
 protected:
  ...
  bool CalcUBSize(int64_t tmpS1BasicBlock, int64_t tmpS2BasicBlock, int64_t batch) override { return true; }
  ...
  uint64_t GetTilingKey() const override { return 0UL; }
};

// NOTE manually initialize tiling data in hostapi scenario in highest priority template
REGISTER_TILING_TEMPLATE("SparseAttentionScore", SparseAttentionScoreTilingDropMask, 0);
REGISTER_TILING_TEMPLATE("SparseAttentionScore", SparseAttentionVarLenScoreTiling, 94);
REGISTER_TILING_TEMPLATE("SparseAttentionScore", SparseAttentionScoreTilingS1s2Bn2gs1, 96);
REGISTER_TILING_TEMPLATE("SparseAttentionScore", SparseAttentionScoreTilingS1Bn2gs1, 97);
REGISTER_TILING_TEMPLATE("SparseAttentionScore", SparseAttentionScoreTilingB, 98);
}  // namespace FA
}  // namespace optiling
