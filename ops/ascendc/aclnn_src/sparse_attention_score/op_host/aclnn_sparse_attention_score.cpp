...
static constexpr int64_t PAD_BASIC_BLOCK = 16;
static constexpr int64_t PAD_LOWER_BOUND_196 = 196;
static constexpr int64_t PAD_ALIGN_128 = 128;
static constexpr int64_t PAD_ALIGN_SPL_SHAPE = 448;
static constexpr int64_t MAX_STRIDE_S1 = 65535;
static constexpr uint64_t DIM_NUM_4 = 4;
static constexpr uint64_t DIM_NUM_3 = 3;
static constexpr uint64_t DIM_NUM_2 = 2;
static constexpr int64_t HEAD_DIM_MAX = 512;
static constexpr int64_t PSE_TYPE_V1 = 1;  // add and mul
static constexpr int64_t PSE_INNER_MUL_ADD = 2;
static constexpr int64_t PSE_INNER_MUL_ADD_SQRT = 3;
static constexpr int64_t HEAD_DIM_72 = 72;
static constexpr int64_t HEAD_DIM_88 = 88;
static constexpr int64_t SEQ_LEN_1024 = 1024;
static constexpr int64_t TND_UNPAD_MAX_S2 = 1024;
static constexpr int64_t TND_UNPAD_MAX_S1_SUM = 160 * 1024;
static constexpr int64_t TND_UNPAD_MAX_DDIM = 96;
...
static inline void AnalysisAxisForBsh(const Shape &qShape, const Shape &kShape, FaShapeInfo &shapeInfo) {
...
static inline void AnalysisAxisForBsnd(const Shape &qShape, const Shape &kShape, FaShapeInfo &shapeInfo) {
...
static inline void AnalysisAxisForTnd(const Shape &qShape, const Shape &kShape, FaShapeInfo &shapeInfo) {
...
static inline void AnalysisAxisForSbh(const Shape &qShape, const Shape &kShape, FaShapeInfo &shapeInfo) {
...
static inline void AnalysisAxisForBnsd(const Shape &qShape, const Shape &kShape, FaShapeInfo &shapeInfo) {
...
static inline aclnnStatus AnalysisAxis(const aclTensor *query, const aclTensor *key, const char *inputLayout,
                                       int64_t headNum, FaShapeInfo &shapeInfo) {
...
static inline void SetShapeInfoForBshBsnd(int64_t alignedH1Size, FaShapeInfo &shapeInfo) {
  ...
  if (shapeInfo.needReshape) {
    ...
    shapeInfo.reshapedKeyValueShape.assign({shapeInfo.axes.b, shapeInfo.axes.s2, shapeInfo.axes.n2, shapeInfo.axes.d});
  }
}

static inline void SetShapeInfoForSbh(int64_t alignedH1Size, FaShapeInfo &shapeInfo) {
  ...
  if (shapeInfo.needReshape) {
    ...
    shapeInfo.reshapedKeyValueShape.assign({shapeInfo.axes.s2, shapeInfo.axes.b, shapeInfo.axes.n2, shapeInfo.axes.d});
  }
}

static inline int64_t GetSumIntArrayMaxValue(const aclIntArray *intArrayValue) {
  ...
  for (size_t i = 1; i < intArrayValue->Size(); ++i) {
    int64_t tmpMaxLength = static_cast<int64_t>((*intArrayValue)[i]) - static_cast<int64_t>((*intArrayValue)[i - 1]);
...
static inline bool IsNeedPad(const FaShapeInfo &shapeInfo, const aclIntArray *actualSeqQLenOptional,
                             const aclIntArray *actualSeqKvLenOptional) {
  if ((shapeInfo.axes.d == HEAD_DIM_72 || shapeInfo.axes.d == HEAD_DIM_88) && shapeInfo.axes.s2 <= SEQ_LEN_1024 &&
      shapeInfo.inputLayout != InputLayout::BNSD && shapeInfo.inputLayout != InputLayout::TND &&
      shapeInfo.axes.n1 == shapeInfo.axes.n2 && shapeInfo.needTranspose == false) {
    return false;
  }
  ...
  if (shapeInfo.inputLayout == InputLayout::TND) {
    ...
    if (actualSeqQLenOptional != nullptr && actualSeqKvLenOptional != nullptr &&
      ...
      sQLenSum = actualSeqQLenOptional->Size() >= 1
                   ? static_cast<int64_t>((*actualSeqQLenOptional)[actualSeqQLenOptional->Size() - 1])
                   : 0;
    }
...
static inline aclnnStatus InputDtypeCheck(const aclTensor *query, const aclTensor *key, const aclTensor *value,
  ...
  if (pseTypeOptional == PSE_INNER_MUL_ADD ||
      pseTypeOptional == PSE_INNER_MUL_ADD_SQRT) {  // Inner pse alibi, dtype must be fp32
...
static inline aclnnStatus AnalysisInput(const aclTensor *query, const aclTensor *key, char *inputLayout,
                                        int64_t headNum, FaShapeInfo &shapeInfo,
  ...
  OP_LOGD(
    "Analysis input success. The analysis result: [needReshape]: %d, [needPad]: %d, [padNum]: %lu,"
    "[needTranspose]: %d.",
...
static inline aclnnStatus Contiguous(const aclTensor *&query, const aclTensor *&key, const aclTensor *&value,
                                     ...
                                     const aclTensor *&selectBlockIdxOptional, aclOpExecutor *executor) {
  ...
  if (selectBlockIdxOptional) {
    selectBlockIdxOptional = l0op::Contiguous(selectBlockIdxOptional, executor);
    CHECK_RET(selectBlockIdxOptional != nullptr, ACLNN_ERR_INNER_NULLPTR);
  }
  return ACLNN_SUCCESS;
}

static inline aclnnStatus PreprocessQKV(const aclTensor *&query, const aclTensor *&key, const aclTensor *&value,
  ...
  if (shapeInfo.needReshape) {
    ...
    CHECK_RET(query != nullptr, ACLNN_ERR_INNER_NULLPTR);
    key = l0op::Reshape(
      key, executor->AllocIntArray(shapeInfo.reshapedKeyValueShape.data(), shapeInfo.reshapedKeyValueShape.size()),
      executor);
    CHECK_RET(key != nullptr, ACLNN_ERR_INNER_NULLPTR);
    value = l0op::Reshape(
      value, executor->AllocIntArray(shapeInfo.reshapedKeyValueShape.data(), shapeInfo.reshapedKeyValueShape.size()),
      executor);
    CHECK_RET(value != nullptr, ACLNN_ERR_INNER_NULLPTR);
  }
  ...
  if (shapeInfo.inputLayout == InputLayout::SBH && shapeInfo.needPad && !shapeInfo.needTranspose) {
    // (S,B,N,D) -> (S,B,N*D)
    FVector<int64_t, DIM_NUM_3> queryShape{
      shapeInfo.axes.s1, shapeInfo.axes.b,
      shapeInfo.axes.n1 * (shapeInfo.axes.d + static_cast<int64_t>(shapeInfo.padNum))};
    FVector<int64_t, DIM_NUM_3> keyValueShape{
      shapeInfo.axes.s2, shapeInfo.axes.b,
...
static inline aclnnStatus Postprocess(const aclTensor *&l0AttentionOutOut, const aclTensor *attentionOutOut,
  ...
  if (shapeInfo.needReshape) {
    ...
    l0AttentionOutOut = l0op::Reshape(
      l0AttentionOutOut, executor->AllocIntArray(attentionOutOutShape.data(), attentionOutOutShape.size()), executor);
    CHECK_RET(l0AttentionOutOut != nullptr, ACLNN_ERR_INNER_NULLPTR);
  }
  return ACLNN_SUCCESS;
}

static inline aclnnStatus CheckFaParam(const aclTensor *query, const aclTensor *key, const aclTensor *value,
                                       const char *inputLayout, const aclTensor *softmaxMaxOut,
                                       const aclTensor *softmaxSumOut, const aclTensor *attentionOutOut,
...
extern "C" aclnnStatus aclnnSparseAttentionScoreMSGetWorkspaceSize(
  ...
  CHECK_RET(CheckFaParam(query, key, value, inputLayout, softmaxMaxOut, softmaxSumOut, attentionOutOut, workspaceSize,
                         executor) == ACLNN_SUCCESS,
            ACLNN_ERR_INNER_NULLPTR);
  L2_DFX_PHASE_1(
    aclnnSparseAttentionScoreMS,
    DFX_IN(query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional, attenMaskOptional,
           prefixOptional, selectBlockIdxOptional, scaleValueOptional, keepProbOptional, preTokensOptional,
           nextTokensOptional, headNum, inputLayout, innerPreciseOptional, sparseModeOptional, selectBlockLenOptional),
  ...
  CHECK_RET(InputDtypeCheck(query, key, value, realShiftOptional, PSE_TYPE_V1) == ACLNN_SUCCESS,
  ...
  CHECK_RET(Contiguous(query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional, attenMaskOptional,
                       selectBlockIdxOptional, l0Executor) == ACLNN_SUCCESS,
            ACLNN_ERR_INNER_NULLPTR);

  CHECK_RET(PreprocessQKV(query, key, value, shapeInfo, l0Executor) == ACLNN_SUCCESS, ACLNN_ERR_INNER_NULLPTR);

  auto l0SparseAttentionScoreOuts = l0op::SparseAttentionScore(
    query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional, attenMaskOptional, prefixOptional,
    nullptr, nullptr, nullptr, nullptr, selectBlockIdxOptional, scaleValueOptional, keepProbOptional, preTokensOptional,
    nextTokensOptional, headNum, shapeInfo.l0InputLayoutStr.c_str(), innerPreciseOptional, sparseModeOptional,
    PSE_TYPE_V1, selectBlockLenOptional, l0Executor);

  CHECK_RET(l0SparseAttentionScoreOuts[0] != nullptr, ACLNN_ERR_INNER_NULLPTR);
  CHECK_RET(l0SparseAttentionScoreOuts[1] != nullptr, ACLNN_ERR_INNER_NULLPTR);
  CHECK_RET(l0SparseAttentionScoreOuts[3] != nullptr, ACLNN_ERR_INNER_NULLPTR);
  auto l0SoftmaxMaxOut = l0SparseAttentionScoreOuts[0];
  auto l0SoftmaxSumOut = l0SparseAttentionScoreOuts[1];
  // l0SoftmaxOutOut not used now
  auto l0AttentionOutOut = l0SparseAttentionScoreOuts[3];
...
extern "C" aclnnStatus aclnnSparseAttentionScoreMS(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                                 const aclrtStream stream) {
  L2_DFX_PHASE_2(aclnnSparseAttentionScoreMS);
  // 固定写法，调用框架能力，完成计算
  return CommonOpExecutorRun(workspace, workspaceSize, executor, stream);
}

extern "C" aclnnStatus aclnnSparseAttentionScoreMSV2GetWorkspaceSize(
  const aclTensor *query, const aclTensor *key, const aclTensor *value, const aclTensor *realShiftOptional,
  const aclTensor *dropMaskOptional, const aclTensor *paddingMaskOptional, const aclTensor *attenMaskOptional,
  ...
  CHECK_RET(CheckFaParam(query, key, value, inputLayout, softmaxMaxOut, softmaxSumOut, attentionOutOut, workspaceSize,
                         executor) == ACLNN_SUCCESS,
            ACLNN_ERR_INNER_NULLPTR);
  L2_DFX_PHASE_1(aclnnSparseAttentionScoreMSV2,
                 DFX_IN(query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional, attenMaskOptional,
                        prefixOptional, qStartIdxOptional, kvStartIdxOptional, selectBlockIdxOptional,
                        scaleValueOptional, keepProbOptional, preTokensOptional, nextTokensOptional, headNum,
                        inputLayout, innerPreciseOptional, sparseModeOptional, pseTypeOptional, selectBlockLenOptional),
                 DFX_OUT(softmaxMaxOut, softmaxSumOut, softmaxOutOut, attentionOutOut));
  ...
  auto l0SparseAttentionScoreOuts = l0op::SparseAttentionScore(
    query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional, attenMaskOptional, prefixOptional,
    nullptr, nullptr, qStartIdxOptional, kvStartIdxOptional, selectBlockIdxOptional, scaleValueOptional,
    keepProbOptional, preTokensOptional, nextTokensOptional, headNum, shapeInfo.l0InputLayoutStr.c_str(),
    innerPreciseOptional, sparseModeOptional, pseTypeOptional, selectBlockLenOptional, l0Executor);
...
extern "C" aclnnStatus aclnnSparseAttentionScoreMSV2(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                                   const aclrtStream stream) {
  L2_DFX_PHASE_2(aclnnSparseAttentionScoreMSV2);
  // 固定写法，调用框架能力，完成计算
  return CommonOpExecutorRun(workspace, workspaceSize, executor, stream);
}
