...
const std::array<const aclTensor *, 4> SparseAttentionScore(
  ...
  const aclIntArray *kvStartIdxOptional, const aclTensor *selectBlockIdxOptional, double scaleValueOptional,
  double keepProbOptional, int64_t preTockensOptional, int64_t nextTockensOptional, int64_t headNum,
  const char *inputLayout, int64_t innerPreciseOptional, int64_t sparseModeOptional, int64_t pseTypeOptional,
  uint32_t selectBlockLenOptional, aclOpExecutor *executor) {
  L0_DFX(SparseAttentionScore, query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional,
         attenMaskOptional, prefixOptional, actualSeqQLenOptional, actualSeqKvLenOptional, qStartIdxOptional,
         kvStartIdxOptional, selectBlockIdxOptional, scaleValueOptional, keepProbOptional, preTockensOptional,
         nextTockensOptional, headNum, inputLayout, innerPreciseOptional, sparseModeOptional, pseTypeOptional,
         selectBlockLenOptional);
  ...
  if (selectBlockIdxOptional == nullptr) {
    selectBlockIdxOptional = executor->AllocTensor(DataType::DT_INT32, Format::FORMAT_ND, Format::FORMAT_ND);
  }
  ...
  auto ret = INFER_SHAPE(SparseAttentionScore,
                         OP_INPUT(query, key, value, realShiftOptional, dropMaskOptional, paddingMaskOptional,
                                  ...
                                  qStartIdxOptionalTensor, kvStartIdxOptionalTensor, selectBlockIdxOptional),
                         ...
                         OP_ATTR(static_cast<float>(scaleValueOptional), static_cast<float>(keepProbOptional),
                                 ...
                                 sparseModeOptional, pseTypeOptional, selectBlockLenOptional));
  if (ret != ACLNN_SUCCESS) {
    OP_LOGE(ACLNN_ERR_PARAM_INVALID, "SparseAttentionScore InferShape failed.");
