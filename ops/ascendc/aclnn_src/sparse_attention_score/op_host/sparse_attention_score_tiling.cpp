...
/*!
 * \file sparse_attention_score_tiling.cpp
 * \brief
 */
...
class SparseAttentionScoreEmptyInputTiling {
 public:
  SparseAttentionScoreTilingData tilingData;

  void SparseAttentionScoreSetEmptyInputTilingData(gert::TilingContext *context,
                                                   SparseAttentionScoreTilingData &faTilingData);
  void GetTilingKeyAttentionScore4EmptyInput(uint32_t &tilingKey, const gert::TilingContext *context);
};

void SparseAttentionScoreEmptyInputTiling::GetTilingKeyAttentionScore4EmptyInput(uint32_t &tilingKey,
...
void SparseAttentionScoreEmptyInputTiling::SparseAttentionScoreSetEmptyInputTilingData(
  gert::TilingContext *context, SparseAttentionScoreTilingData &faTilingData) {
...
static bool IsEmptyInput(gert::TilingContext *context) {
  ...
  if ((queryShapeSize == 0 || keyShapeSize == 0) && (attentionOutShapeSize != 0 || softmaxSumShapeSize != 0)) {
    ...
    */
    auto kernelType = context->GetInputDesc(KEY_INPUT_INDEX)->GetDataType();
    SparseAttentionScoreEmptyInputTiling emptyInputTiling;
    auto compileInfoPtr = reinterpret_cast<const SparseAttentionScoreCompileInfo *>(context->GetCompileInfo());
    OPS_ERR_IF(compileInfoPtr == nullptr, OPS_REPORT_VECTOR_INNER_ERR(context, "compileInfoPtr is null"), return false);
    uint32_t coreNum = compileInfoPtr->aivNum;
    OPS_ERR_IF((coreNum <= 0), OPS_REPORT_VECTOR_INNER_ERR(context, "platform info is invalid, coreNum=%u.", coreNum),
               return false);
    ...
    if (attentionOutShapeSize != 0) {
      if (attentionOutBlockSize % coreNum == 0) {
        ...
        attentionOutLastCoreDataSize =
          attentionOutSingleCoreDataSize -
          (attentionOutBlockSize * MIN_COPY_UINT_SIZE / ge::GetSizeByDataType(kernelType) - attentionOutShapeSize);
        ...
        if (attentionOutBlockSize > coreNum) {
          ...
          attentionOutLastCoreDataSize =
            attentionOutTailCoreDataSize - (attentionOutSingleCoreDataSize * attentionOutFormerNum +
    ...
    emptyInputTiling.tilingData.emptyInputTilingData.set_attentionOutSingleCoreDataSize(attentionOutSingleCoreDataSize);
    ...
    emptyInputTiling.SparseAttentionScoreSetEmptyInputTilingData(context, emptyInputTiling.tilingData);
...
ASCENDC_EXTERN_C ge::graphStatus TilingPrepareForSparseAttentionScore(gert::TilingParseContext *context) {
  ...
  auto compileInfoPtr = context->GetCompiledInfo<SparseAttentionScoreCompileInfo>();
...
IMPL_OP(SparseAttentionScore)
.Tiling(TilingSparseAttentionScore)
.TilingInputsDataDependency({7, 8, 9, 10, 11})
.TilingParse<SparseAttentionScoreCompileInfo>(TilingPrepareForSparseAttentionScore);  // 向框架注册入口函数
