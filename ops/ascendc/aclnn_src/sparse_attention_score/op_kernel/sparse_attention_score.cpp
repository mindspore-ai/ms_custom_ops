...
/*!
 * \file sparse_attention_score.cpp
 * \brief
 */

#include "kernel_operator.h"
#include "sparse_attention_score_empty_tensor.h"
#include "sparse_attention_score_drop_mask_adapter.h"
#include "sparse_attention_score_s1s2_bn2gs1.h"
#include "sparse_attention_score_s1_bn2gs1.h"
#include "sparse_attention_score_bn2gs1s2_b.h"
...
#define COPY_TILING_DATA(tiling)                                                                            \
  GET_TILING_DATA_MEMBER(SparseAttentionScoreGeneralTilingData, bmm1TilingData, bmm1TilingDataVar, tiling); \
  GET_TILING_DATA_MEMBER(SparseAttentionScoreGeneralTilingData, bmm2TilingData, bmm2TilingDataVar, tiling); \
  const SparseAttentionScoreGeneralTilingData *__restrict tilingData = nullptr;                             \
...
#define INVOKE_FA_GENERAL_OP_IMPL_BMM2NZ(templateClass, ...)                                                          \
  do {                                                                                                                \
    templateClass<__VA_ARGS__> op;                                                                                    \
    COPY_TILING_DATA(tiling);                                                                                         \
    REGIST_MATMUL_OBJ(&tPipe, GetSysWorkSpacePtr(), op.bmm1, bmm1tiling, op.bmm2, bmm2tiling, op.bmm2Nz, bmm2tiling); \
  } while (0)

#else  // VECTOR 实现

#define COPY_TILING_DATA(tiling)                                                            \
  GET_TILING_DATA_WITH_STRUCT(SparseAttentionScoreGeneralTilingData, tilingDataIn, tiling); \
  const SparseAttentionScoreGeneralTilingData *__restrict tilingData = &tilingDataIn;       \
...
#define INVOKE_FA_GENERAL_OP_IMPL(templateClass, ...)                                                       \
  do {                                                                                                      \
    ...
    if (tilingData->inputParams.needDropMaskOp) {                                                           \
      SparseAttentionScoreDropMaskAdapter dropMaskAdapter;                                                  \
      dropMaskAdapter.Init(dropMask, user, tilingData, &tPipe);                                             \
      dropMaskAdapter.Process();                                                                            \
      tPipe.Reset();                                                                                        \
      templateClass<__VA_ARGS__> op;                                                                        \
      REGIST_MATMUL_OBJ(&tPipe, GetSysWorkSpacePtr(), op.bmm1, bmm1tiling, op.bmm2, bmm2tiling);            \
      op.Init(query, key, value, pse, dropMask, paddingMask, prefix, attenMask, selectBlockIdx, softmaxMax, \
...
extern "C" __global__ __aicore__ void sparse_attention_score(
  ...
  __gm__ uint8_t *selectBlockIdx, __gm__ uint8_t *softmaxMax, __gm__ uint8_t *softmaxSum, __gm__ uint8_t *softmaxOut,
  ...
  if (TILING_KEY_IS(90) || TILING_KEY_IS(92) || TILING_KEY_IS(94)) {
    GET_TILING_DATA_WITH_STRUCT(SparseAttentionScoreTilingData, tiling_data_in, tiling);
    const SparseAttentionScoreTilingData *__restrict tiling_data = &tiling_data_in;
    if (TILING_KEY_IS(90)) {
      SparseAttentionScoreEmptyTensor<half> op;
      op.Init(softmaxMax, softmaxSum, attentionOut, tiling_data);
      op.Process();
    } else if (TILING_KEY_IS(92)) {
      SparseAttentionScoreEmptyTensor<float> op;
      op.Init(softmaxMax, softmaxSum, attentionOut, tiling_data);
      op.Process();
    } else if (TILING_KEY_IS(94)) {
      SparseAttentionScoreEmptyTensor<bfloat16_t> op;
      op.Init(softmaxMax, softmaxSum, attentionOut, tiling_data);
      op.Process();
    }
    return;
  ...
  if (TILING_KEY_IS(10000000000220130943UL)) {  // SplitS1S2HighPerf: FLOAT16_PRECISION
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_INVALID_LINE_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true);
    return;
  } else if (TILING_KEY_IS(10000000000221130943UL)) {  // SplitS1S2HighPerf: FLOAT16_PRECISION Bmm1-NZ
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true,
                                     CubeFormat::NZ);
    return;
  } else if (TILING_KEY_IS(10000000000221132943UL)) {  // SplitS1S2HighPerf: FLOAT16_PRECISION Bmm1-NZ
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_INVALID_LINE_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true,
                                     CubeFormat::NZ);
    return;
  } else if (TILING_KEY_IS(10000001000220130943UL)) {  // SplitS1S2HighPerf: FLOAT16_PRECISION s1s2L1Reuse
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM2NZ(SparseAttentionScoreS1Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true);
    return;
  } else if (TILING_KEY_IS(10000000002200132953UL)) {  // SplitBn2gs1s2S1dDBHighPerf: FLOAT16_PRECISION Bmm1-ND
    INVOKE_FA_GENERAL_OP_IMPL_BMM2NZ(SparseAttentionScoreS1Bn2gs1, ImplModeEnum::AA_INVALID_LINE_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM2NZ(SparseAttentionScoreBn2gs1s2B, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true,
                                     LayoutMode::BSNGD);
    return;
  } else if (TILING_KEY_IS(10000000000021132099UL)) {  // SplitBbDBHighPerf: FLOAT16_PRECISION
    INVOKE_FA_GENERAL_OP_IMPL_BMM2NZ(SparseAttentionScoreBn2gs1s2B, ImplModeEnum::AA_INVALID_LINE_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, false, false, half, float, true,
                                     LayoutMode::BSNGD);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, false, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, false, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, false, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, false, false, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, true, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, true, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, false, true, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, true, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, true, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, false, true, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, false, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, false, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, false, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, false, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, false, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, false, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, true, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, true, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, true, false, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, true, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, true, true, half, float, true, CubeFormat::ND, true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, true, true, half, float, true, CubeFormat::ND, true);
    return;
  ...
  if (TILING_KEY_IS(10000000000220120943UL)) {  // SplitS1S2HighPerf: BF16
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, false, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, false, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, false, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, false, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, false, false, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, true, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, true, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, false, true, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, false, true, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, false, true, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, false, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, false, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, false, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, false, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, false, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, false, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BSH, true, true, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_SBH, true, true, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, true, false, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                              LayOutTypeEnum::LAYOUT_BNSD, true, true, true, bfloat16_t, float, true, CubeFormat::ND,
                              true);
    return;
  ...
  if (TILING_KEY_IS(10000000000220110943UL)) {  // SplitS1S2HighPerf: FLOAT32_PRECISION
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, false, true, true, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_SBH, false, true, true, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, true, false, true, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_SBH, true, false, true, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, true, true, false, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_SBH, true, true, false, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BSH, true, true, true, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_SBH, true, true, true, float, float, true, CubeFormat::NZ);
    return;
    ...
    INVOKE_FA_GENERAL_OP_IMPL_BMM1NZ(SparseAttentionScoreS1s2Bn2gs1, ImplModeEnum::AA_HIGH_PRECISION,
                                     LayOutTypeEnum::LAYOUT_BNSD, true, true, true, float, float, true, CubeFormat::NZ);
    return;
