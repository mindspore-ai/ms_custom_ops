...
/*!
 * \file sparse_attention_score_empty_tensor.h
 * \brief
 */

#ifndef SPARSE_ATTENTION_SCORE_EMPTY_TENSOR_H
#define SPARSE_ATTENTION_SCORE_EMPTY_TENSOR_H

#include "kernel_operator.h"
#include "kernel_tiling/kernel_tiling.h"

template <typename INPUT_T>
class SparseAttentionScoreEmptyTensor {
 public:
  __aicore__ inline SparseAttentionScoreEmptyTensor() {}
  __aicore__ inline void Init(__gm__ uint8_t *softmaxMax, __gm__ uint8_t *softmaxSum, __gm__ uint8_t *attentionOut,
                              const SparseAttentionScoreTilingData *__restrict tiling);
 ...
 protected:
  ...
  const SparseAttentionScoreTilingData *__restrict tilingData;
  __aicore__ inline void ComputeEachCore();
};

template <typename INPUT_T>
__aicore__ inline void SparseAttentionScoreEmptyTensor<INPUT_T>::Init(
...
template <typename INPUT_T>
__aicore__ inline void SparseAttentionScoreEmptyTensor<INPUT_T>::ComputeEachCore() {
  ...
  if (attentionOutFormerNum == coreNum || (attentionOutFormerNum + attentionOutTailNum) < coreNum) {
    ...
    if (tmpBlockIdx < attentionOutFormerNum) {
      ...
      AscendC::InitOutput<INPUT_T>(attentionOutGm[attentionOutFormerNum * attentionOutSingleCoreDataSize +
  ...
  if (tmpBlockIdx >= (softmaxMaxFormerNum + softmaxMaxTailNum)) {
    ...
    AscendC::InitOutput<float>(softmaxMaxGm[tmpBlockIdx * softmaxMaxSingleCoreDataSize],
                               tilingData->emptyInputTilingData.softmaxMaxSingleCoreDataSize, static_cast<float>(0.0));
    ...
    AscendC::InitOutput<float>(softmaxMaxGm[softmaxMaxFormerNum * softmaxMaxSingleCoreDataSize +
                               ...
                               tilingData->emptyInputTilingData.softmaxMaxTailCoreDataSize, static_cast<float>(0.0));
...
template <typename INPUT_T>
__aicore__ inline void SparseAttentionScoreEmptyTensor<INPUT_T>::Process() {
  ComputeEachCore();
}

#endif  // SPARSE_ATTENTION_SCORE_EMPTY_TENSOR_H
