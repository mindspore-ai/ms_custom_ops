...
class SparseAttentionScoreS1s2Bn2gs1 {
 ...
 protected:
  ...
  __aicore__ inline uint32_t binarySearch(uint32_t findValue);
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
                               enableL1Reuse>::binarySearch(uint32_t findValue) {
  ...
  while (left < right) {
    ...
    AscendC::printf("======== mid:%u, getValue:%u.\n", mid, this->selectBlockIdxBufferUB.GetValue(mid));
    if (static_cast<uint32_t>(this->selectBlockIdxBufferUB.GetValue(mid)) < findValue) {
      left = mid + 1;
  ...
  if constexpr (enableL1Reuse) {
      ...
      for (int64_t s1InnerIdx = s1StartIdx; s1InnerIdx < loopS1EndIdx; ++s1InnerIdx) {
        ...
        if (notLastTwoLoop) {
          ...
          selectBlockIdxLeft = this->binarySearch(
            s2LoopStartAlign);  /// 从selectBlockIdxGm中获取与[s2oCvDimIdx,
          ...
          selectBlockIdxRight = this->binarySearch(
            s2LoopLimitAlign);  /// 这里返回的是排在最右边的有效block的id序号，如果都小于findValue，则返回selectBlockNum
    ...
    for (int64_t multiCoreInnerIdx = multiCoreInnerOffset; multiCoreInnerIdx < multiCoreInnerLimit;
      ...
      if (notLastTwoLoop) {
        ...
        AscendC::printf(
          "============================\n vec:%d, (multiCoreInnerOffset, multiCoreInnerLimit, multiCoreInnerIdx):%ld, "
          "%ld, %ld, s2BaseNratioSize:%d\n [origin] (s2StartIdx, s2EndIdx):(%d, %d), s2LoopLimit:%d, "
          "(s2LoopStartAlign, s2LoopLimitAlign):(%u, %u)\n",
          this->blockIdx, multiCoreInnerOffset, multiCoreInnerLimit, multiCoreInnerIdx, s2BaseNratioSize, s2StartIdx,
          s2EndIdx, s2LoopLimit, s2LoopStartAlign, s2LoopLimitAlign);
        ...
        selectBlockIdxLeftValue = this->selectBlockIdxBufferUB.GetValue(selectBlockIdxLeft);
        selectBlockIdxRightLastValue = this->selectBlockIdxBufferUB.GetValue(
          selectBlockIdxRight - 1);  /// [selectBlockIdxLeftValue, selectBlockIdxRightLastValue] 之间的某些block需要计算
        ...
        AscendC::printf(
          " [Final]  (s2StartIdx, s2EndIdx):(%d, %d), s2LoopLimit:%d, (selectBlockIdxLeftValue, "
          "selectBlockIdxRightLastValue):(%u, %u), (selectBlockIdxLeft, selectBlockIdxRight):(%ld, %ld)\n",
          s2StartIdx, s2EndIdx, s2LoopLimit, selectBlockIdxLeftValue, selectBlockIdxRightLastValue, selectBlockIdxLeft,
          selectBlockIdxRight);
        AscendC::printf("============================== selectBlockIdxBufferUB: \n");
        for (int j = 0; j < this->selectBlockNum; j++) {
          AscendC::printf("%u, ", this->selectBlockIdxBufferUB.GetValue(j));
        }
        AscendC::printf("\n");
      ...
      for (int64_t selectBlockIdx = selectBlockIdxLeft; selectBlockIdx < selectBlockIdxRight; selectBlockIdx++) {
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld selectLoop begin.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);

        if (notLastTwoLoop) {
          s2LoopCount = this->selectBlockIdxBufferUB.GetValue(selectBlockIdx) - selectBlockIdxLeftValue;
        }
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld, s2LoopCount:%ld.\n",
        ...
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld WaitBmm1Result over.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld IterateBmm1 over.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld ProcessVec1 over.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld WaitBmm2Result over.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld IterateBmm2 over.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);

        if (taskId > 1) {
          this->ProcessVec2(extraInfo[(taskId + 1) % 3]);
        }
        AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld ProcessVec2 over.\n",
                        this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
