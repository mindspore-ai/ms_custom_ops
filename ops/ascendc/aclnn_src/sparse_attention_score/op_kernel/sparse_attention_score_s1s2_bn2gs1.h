...
/*!
 * \file sparse_attention_score_s1s2_bn2gs1.h
 * \brief
 */

#ifndef SPARSE_ATTENTION_SCORE_S1S2_BN2GS1_H
#define SPARSE_ATTENTION_SCORE_S1S2_BN2GS1_H

#include "util.h"
#include "dropmask.h"
#include "sparse_attention_score_common.h"
...
class SparseAttentionScoreS1s2Bn2gs1 {
 public:
  __aicore__ inline SparseAttentionScoreS1s2Bn2gs1() {}
  ...
  __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value, __gm__ uint8_t *pse,
                              ...
                              __gm__ uint8_t *attenMask, __gm__ uint8_t *selectBlockIdx, __gm__ uint8_t *softmaxMax,
                              __gm__ uint8_t *softmaxSum, __gm__ uint8_t *softmaxOut, __gm__ uint8_t *attentionOut,
                              __gm__ uint8_t *workspace, const SparseAttentionScoreGeneralTilingData *__restrict tiling,
                              TPipe *tPipe);
  ...
  using modeTypemm2 =
    typename AscendC::Conditional<(IsSameType<T, INPUT_T>::value == false && layOutType == LayOutTypeEnum::LAYOUT_TND),
 ...
 protected:
  __aicore__ inline void InitInput(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                   ...
                                   __gm__ uint8_t *prefix, __gm__ uint8_t *attenMask, __gm__ uint8_t *selectBlockIdx,
                                   __gm__ uint8_t *softmaxMax, __gm__ uint8_t *softmaxSum, __gm__ uint8_t *softmaxOut,
                                   __gm__ uint8_t *attentionOut, __gm__ uint8_t *workspace,
                                   const SparseAttentionScoreGeneralTilingData *__restrict tiling, TPipe *tPipe);
  ...
  __aicore__ inline void SetTiling(const SparseAttentionScoreGeneralTilingData *__restrict tilingData);
  ...
  __aicore__ inline uint32_t binarySearch(uint32_t findValue, uint64_t offset);
  ...
  int32_t blockIdx;
  const SparseAttentionScoreGeneralTilingData *__restrict tilingData;
  ...
  GlobalTensor<uint32_t> selectBlockIdxGm;
  uint32_t selectBlockLen;
  uint32_t selectBlockNum;
  uint64_t selectBlockOffset;
  TBuf<TPosition::VECCALC> selectBlokcIdVecQue;
  LocalTensor<uint32_t> selectBlockIdxBufferUB;
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
                               enableL1Reuse>::Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                                    ...
                                                    __gm__ uint8_t *attenMask, __gm__ uint8_t *selectBlockIdx,
                                                    __gm__ uint8_t *softmaxMax, __gm__ uint8_t *softmaxSum,
                                                    __gm__ uint8_t *softmaxOut, __gm__ uint8_t *attentionOut,
                                                    __gm__ uint8_t *workspace,
                                                    const SparseAttentionScoreGeneralTilingData *__restrict tiling,
                                                    TPipe *tPipe) {
  this->InitInput(query, key, value, pse, dropMask, paddingMask, prefix, attenMask, selectBlockIdx, softmaxMax,
                  softmaxSum, softmaxOut, attentionOut, workspace, tiling, tPipe);  // gm设置
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
                               enableL1Reuse>::InitInput(__gm__ uint8_t *query, __gm__ uint8_t *key,
                                                         ...
                                                         __gm__ uint8_t *selectBlockIdx, __gm__ uint8_t *softmaxMax,
                                                         __gm__ uint8_t *softmaxSum, __gm__ uint8_t *softmaxOut,
                                                         __gm__ uint8_t *attentionOut, __gm__ uint8_t *workspace,
                                                         const SparseAttentionScoreGeneralTilingData *__restrict tiling,
                                                         TPipe *tPipe) {
  // AscendC::printf("==================== s1s2_BN2GS1: InitInput  =====================\n");
  ...
  this->selectBlockIdxGm.SetGlobalBuffer((__gm__ uint32_t *)selectBlockIdx);
  // AscendC::printf("============================== selectBlockIdxGm: \n");
  // for(int i=0;i<64;i++)
  // {
  //     for(int j=0;j<8;j++)
  //     {
  //         AscendC::printf("%u, ", this->selectBlockIdxGm.GetValue(i*8+j));
  //     }
  //     AscendC::printf("\n");
  // }
  ...
  this->stage1Res[0].SetGlobalBuffer((__gm__ INPUT_T *)(workspace + this->blockIdx * totalOffset + vector1OffsetPing));
  this->stage1Res[1].SetGlobalBuffer((__gm__ INPUT_T *)(workspace + this->blockIdx * totalOffset + vector1OffsetPong));
  ...
  this->selectBlockLen = this->tilingData->inputParams.selectBlockLen;
  this->selectBlockNum = this->tilingData->inputParams.selectBlockNum;
}

template <ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasPse, bool hasAtten, bool hasDrop, typename INPUT_T,
          typename T, bool isBasicBlock, CubeFormat bmm1Format, bool enableL1Reuse>
__aicore__ inline void SparseAttentionScoreS1s2Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  enableL1Reuse>::SetTiling(const SparseAttentionScoreGeneralTilingData *__restrict tilingData) {
  // copy base params
...
template <ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasPse, bool hasAtten, bool hasDrop, typename INPUT_T,
          typename T, bool isBasicBlock, CubeFormat bmm1Format, bool enableL1Reuse>
__aicore__ inline void SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
  ...
  this->pipe->InitBuffer(this->selectBlokcIdVecQue,
                         sizeof(uint32_t) * this->selectBlockNum);  /// 为selectBlockIdx一行数据的大小
  this->selectBlockIdxBufferUB = this->selectBlokcIdVecQue.template Get<uint32_t>();  ///
}
...
__aicore__ inline void SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
  ...
  if constexpr (hasPse == true) {
    ...
    this->pseInfo.pseEncodeType = static_cast<uint32_t>(this->tilingData->inputParams.pseEncodeType);
...
/**
 * @description:
 * 二分查找函数，在selectBlockIdxGm中找到大于等于findValue的最小那个数，然后返回这个数的id号，如果selectBlockIdxGm中所有的数都小于findValue，则返回selectBlockIdxGm的size；通用，sag不用整改
 * @param {GlobalTensor<T1>} selectBlockIdxGm  在[selectBlockOffset, selectBlockOffset +
 * selectBlockNum]范围内已经排好序（升序）
 * @param {T1} findValue
 * @param {uint64_t} selectBlockNum
 * @return {*}  返回 selectBlockIdxGm 中大于等于findValue的最左边的那个数的id序号
 * @Date: 2025-03-28 03:37:13
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
                               enableL1Reuse>::binarySearch(uint32_t findValue, uint64_t offset) {
  // AscendC::printf("==== binarySearch. findValue:%u\n", findValue);
  uint32_t left = 0;
  uint32_t right = this->selectBlockNum;
  uint64_t mid;
  // AscendC::printf("======== begin: left:%u, right:%u.\n", left, right);

  while (left < right) {
    mid = left + (right - left) / 2;
    // AscendC::printf("======== mid:%u, getValue:%u.\n", mid, this->selectBlockIdxGm.GetValue(offset+mid));
    if (static_cast<uint32_t>(this->selectBlockIdxGm.GetValue(offset + mid)) < findValue) {
      left = mid + 1;
      // AscendC::printf("======== getValue < findValue. Let left=mid+1=%u.\n", left);
    } else {
      right = mid;
      // AscendC::printf("======== getValue >= findValue. Let right=mid=%u.\n", right);
    }
  }
  // AscendC::printf("======== return: left: %u, right:%u.\n", left, right);
  return left;
}
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if (this->tilingData->inputParams.sparseType == static_cast<uint8_t>(SparseModeEnum::CAUSAL)) {  // 下三角
    ...
    if constexpr (enableL1Reuse) {
      if (useNext) {
        this->nextS2EndIdx =
          Min((this->s1oIdx + 2) * this->s1BaseSize + this->tilingData->coreParams.s2SparseValidSize, this->s2Size);
      }
    }
    ...
    if constexpr (enableL1Reuse) {
      if (useNext) {
        this->nextS2EndIdx = Max(s1BaseSize * (this->s1oIdx + 2) - this->s1Size + this->s2Size,
                                 ((__gm__ int64_t *)this->prefixNAddr)[this->boIdx]);
      }
    }
...
__aicore__ inline void SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
  ...
  if constexpr (enableL1Reuse) {
    ...
    bool notLast = true;
    // AscendC::printf("==== vec:%d, EnableL1Reuse begin.\n",this->blockIdx);
    for (int64_t bngIdx = bngStartIdx; bngIdx < bngEndIdx; ++bngIdx) {
      ...
      // AscendC::printf("======== vec:%d, For loop begin.\n",this->blockIdx);
      for (int64_t s1InnerIdx = s1StartIdx; s1InnerIdx < loopS1EndIdx; ++s1InnerIdx) {
        ...
        int64_t s2LoopLimit;
        uint64_t selectBlockOffset;
        uint32_t selectBlockIdxLeft, selectBlockIdxRight;
        uint32_t s2LoopStartAlign, s2LoopLimitAlign;
        int64_t multiCorePingPong;
        this->softMaxCheckRes = SOFTMAX_CHECK_RES_DEFAULT_VALUE;
        // AscendC::printf("===== BinarySearch begin.\n");
        if (notLastTwoLoop) {
          pingPongCount++;
          if ((s1InnerIdx - s1StartIdx) % 2 == blockMod2Equal0) {
            continue;
          }
          // AscendC::printf("============ vec:%d, (bngIdx, s1InnerIdx):(%d, %d) (s1StartIdx, loopS1EndIdx):(%d, %d)
          // notLastTwoLoop begin...\n",this->blockIdx, bngIdx, s1InnerIdx, s1StartIdx, loopS1EndIdx);
          ...
          s2LoopLimit = CeilDiv(this->s2EndIdx - this->s2StartIdx, s2BaseNratioSize) - 1;  /// origin
          multiCorePingPong = pingPongCount / 2;

          s2LoopStartAlign = static_cast<uint32_t>(this->s2StartIdx /
                                                   s2BaseNratioSize);  /// 对s2方向进行block切分，且偏移对齐至block_size
          s2LoopLimitAlign = static_cast<uint32_t>(
            CeilDiv(this->s2EndIdx, s2BaseNratioSize));  /// 从 [s2LoopStartAlign, s2LoopLimitAlign) 为需要计算的
          selectBlockOffset = static_cast<uint64_t>((bngIdx * this->s1OuterSize + s1InnerIdx) *
                                                    this->selectBlockNum);  /// 当前s1值时，selectBlock的开始偏移地址
          DataCopy(this->selectBlockIdxBufferUB, this->selectBlockIdxGm[selectBlockOffset], this->selectBlockNum);
          selectBlockIdxLeft = this->binarySearch(
            s2LoopStartAlign,
            selectBlockOffset);  /// 从selectBlockIdxGm中获取与[s2oCvDimIdx,
                                 /// s2oIdxDown]有交集的有效block，这里返回的是排在最左边的有效block的id序号，
          /// [selectBlockIdxLeft, selectBlockIdxRight)需要计算
          selectBlockIdxRight = this->binarySearch(
            s2LoopLimitAlign,
            selectBlockOffset);
          /// 这里返回的是排在最右边的有效block的id序号，如果都小于findValue，则返回selectBlockNum
          // AscendC::printf("=============\n blockIdx: %d\n bngStartIdx:%d, bngEndIdx:%d, bngIdx:%d\n s1StartIdx:%d,
          // loopS1EndIdx:%d, s1InnerIdx:%d, selectBlockOffset:%lu\n", this->blockIdx, bngStartIdx, bngEndIdx, bngIdx,
          // s1StartIdx, loopS1EndIdx, s1InnerIdx, selectBlockOffset); AscendC::printf("===== (s2BaseNratioSize,
          // s1OuterSize, selectBlockNum, bngIdx, s1InnerIdx):(%ld, %d, %ld, %ld, %d).\n", s2BaseNratioSize,
          // this->s1OuterSize, this->selectBlockNum, bngIdx, s1InnerIdx); AscendC::printf("============ vec:%d,
          // (bngIdx, s1InnerIdx):(%d, %d) BinarySearch over: (s2StartIdx, s2EndIdx, s2LoopLimit):(%d, %d, %d)
          // (s2LoopStartAlign, s2LoopLimitAlign):(%u,%u) (selectBlockIdxLeft, selectBlockIdxRight):(%lu, %lu).\n",
          //                 this->blockIdx,           bngIdx, s1InnerIdx,               this->s2StartIdx,
          //                 this->s2EndIdx, s2LoopLimit,                   s2LoopStartAlign, s2LoopLimitAlign,
          //                 selectBlockIdxLeft, selectBlockIdxRight);
          s2LoopLimit = static_cast<int64_t>(selectBlockIdxRight - selectBlockIdxLeft - 1);
          this->s2EndIdx = Min((s2LoopLimit + 1) * s2BaseNratioSize, this->s2Size);
        } else {
          s2LoopLimit = 0;
          // selectBlockIdxLeft = 0;
          // selectBlockIdxRight = 1;
          // AscendC::printf("============ vec:%d, (bngIdx, s1InnerIdx):(%d, %d)  LastTwoLoop: (s2StartIdx, s2EndIdx,
          // s2LoopLimit):(%d, %d, %d)  (selectBlockIdxLeft, selectBlockIdxRight):(%lu, %lu).\n",
          //                 this->blockIdx,           bngIdx, s1InnerIdx,               this->s2StartIdx,
          //                 this->s2EndIdx, s2LoopLimit,                       selectBlockIdxLeft,
          //                 selectBlockIdxRight);
        }

        // AscendC::printf("============ vec:%d, (bngIdx, s1InnerIdx):(%d, %d),  s2Loop begin...\n", this->blockIdx,
        // bngIdx, s1InnerIdx);
        for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; s2LoopCount++) {
          // int64_t s2LoopCount = 0;
          // for (uint64_t selectBlockIdx = selectBlockIdxLeft; selectBlockIdx < selectBlockIdxRight; selectBlockIdx++)
          // {
          //     AscendC::printf("================  vec:%d: GetValue: idx(%ld), ", this->blockIdx, selectBlockIdx);
          //     if(notLastTwoLoop){
          //         s2LoopCount  = static_cast<int64_t>(this->selectBlockIdxBufferUB.GetValue(selectBlockIdx));
          //         AscendC::printf("value(%ld), ", s2LoopCount);
          //     }
          //     s2LoopCount  = static_cast<int64_t>(selectBlockIdx);
          //     AscendC::printf("selectBlockIdx(%ld)\n", s2LoopCount);

          if (taskId > 0 && notLast) {
            // 对应extraInfo[(i+2)%3]
            WaitBmm1Result(extraInfo[(taskId + 2) % 3]);
          }
          // AscendC::printf("================  vec:%d, waitBmm1Result over.\n",this->blockIdx);
          if (notLastTwoLoop) {
            this->SetExtraInfo(extraInfo[taskId % 3], taskId, s2LoopCount, s2LoopLimit, multiCorePingPong, lastNotPair);
            if constexpr (bmm1Format == CubeFormat::NZ) {
          ...
          // AscendC::printf("================  vec:%d, IterateBmm1 over\n",this->blockIdx);

          if (taskId > 0 && notLast) {
            this->ProcessVec1(extraInfo[(taskId + 2) % 3]);
            SetFlag<HardEvent::MTE3_MTE2>(eventIdMte3ToMte2);
          }
          // AscendC::printf("================  vec:%d, ProcessVec1 over\n",this->blockIdx);

          if (taskId > 1) {
            // 对应extraInfo[(i+1)%3]
            WaitBmm2Result();
          }
          // AscendC::printf("================  vec:%d, WaitBmm2Result over\n",this->blockIdx);

          if (taskId > 0 && notLast) {
            WaitFlag<HardEvent::MTE3_MTE2>(eventIdMte3ToMte2);
            this->IterateBmm2(extraInfo[(taskId + 2) % 3]);
          }
          // AscendC::printf("================  vec:%d, IterateBmm2 over\n",this->blockIdx);

          if (taskId > 1) {
            this->ProcessVec2(extraInfo[(taskId + 1) % 3]);
          }
          // AscendC::printf("================  vec:%d, ProcessVec2 over\n",this->blockIdx);
          taskId++;
        }
      }
      // AscendC::printf("======== vec:%d, For loop over.\n",this->blockIdx);
    }
    // AscendC::printf("==== vec:%d, EnableL1Reuse over.\n",this->blockIdx);
  } else {
    // AscendC::printf("==== vec:%d, No enableL1Reuse begin.\n",this->blockIdx);
    bool notSecondLast = true;
    bool notLast = true;
    ...
    for (int64_t multiCoreInnerIdx = multiCoreInnerOffset; multiCoreInnerIdx < multiCoreInnerLimit;
      ...
      this->softMaxCheckRes = SOFTMAX_CHECK_RES_DEFAULT_VALUE;
      bool notLastTwoLoop = notSecondLast && notLast;
      uint64_t selectBlockIdxLeft, selectBlockIdxRight;  /// 序号
      uint32_t selectBlockIdxLeftValue,
        selectBlockIdxRightLastValue;  /// 上面selectBlockIdxLeft、(selectBlockIdxRight-1)序号所对应的值
      uint64_t selectBlockOffset;
      if (notLastTwoLoop) {
        ...
        selectBlockOffset =
          static_cast<uint64_t>(multiCoreInnerIdx * this->selectBlockNum);  /// 当前s1o值时，selectBlock的开始偏移地址
        uint32_t s2LoopStartAlign =
          static_cast<uint32_t>(this->s2StartIdx / s2BaseNratioSize);  /// 对s2方向进行block切分，且偏移对齐至block_size
        uint32_t s2LoopLimitAlign = static_cast<uint32_t>(
          CeilDiv(this->s2EndIdx, s2BaseNratioSize));  /// 从 [s2LoopStartAlign, s2LoopLimitAlign) 为需要计算的

        // AscendC::printf("============================\n vec:%d, (multiCoreInnerOffset, multiCoreInnerLimit,
        // multiCoreInnerIdx):%ld, %ld, %ld, s2BaseNratioSize:%d\n [origin] (s2StartIdx, s2EndIdx):(%d, %d),
        // s2LoopLimit:%d, (s2LoopStartAlign, s2LoopLimitAlign):(%u, %u)\n",
        //                                         this->blockIdx,  multiCoreInnerOffset, multiCoreInnerLimit,
        //                                         multiCoreInnerIdx,                s2BaseNratioSize, s2StartIdx,
        //                                         s2EndIdx,            s2LoopLimit,    s2LoopStartAlign,
        //                                         s2LoopLimitAlign);
        ...
        selectBlockIdxRight = this->binarySearch(
          s2LoopLimitAlign,
          selectBlockOffset);  /// 这里返回的是排在最右边的有效block的id序号，如果都小于findValue，则返回selectBlockNum
        // AscendC::printf(" [Final]  (s2LoopStartAlign, s2LoopLimitAlign):(%u, %u), (selectBlockIdxLeft,
        // selectBlockIdxRight):(%ld, %ld)\n",
        //                    s2LoopStartAlign, s2LoopLimitAlign,             selectBlockIdxLeft, selectBlockIdxRight);

        selectBlockIdxLeftValue = this->selectBlockIdxGm.GetValue(selectBlockOffset + selectBlockIdxLeft);
        selectBlockIdxRightLastValue = this->selectBlockIdxGm.GetValue(
          selectBlockOffset + selectBlockIdxRight -
          1);  /// [selectBlockIdxLeftValue, selectBlockIdxRightLastValue] 之间的某些block需要计算
        this->s2StartIdx = selectBlockIdxLeftValue * this->s2BaseNratioSize;  /// 筛选后，最左边block的s2方向的开始地址
        this->s2EndIdx = Min((selectBlockIdxRightLastValue + 1) * this->s2BaseNratioSize, this->s2Size);
        s2LoopLimit = static_cast<int64_t>(selectBlockIdxRightLastValue - selectBlockIdxLeftValue);

        // AscendC::printf(" [Final]  (s2StartIdx, s2EndIdx):(%d, %d), s2LoopLimit:%d, (selectBlockIdxLeftValue,
        // selectBlockIdxRightLastValue):(%u, %u), (selectBlockIdxLeft, selectBlockIdxRight):(%ld, %ld)\n",
        //                    s2StartIdx, s2EndIdx,                   s2LoopLimit,     selectBlockIdxLeftValue,
        //                    selectBlockIdxRightLastValue,             selectBlockIdxLeft, selectBlockIdxRight);
        // AscendC::printf("============================== selectBlockIdxBufferUB: \n");
        // for(int j=0;j<this->selectBlockNum;j++)
        // {
        //     AscendC::printf("%u, ", this->selectBlockIdxGm.GetValue(selectBlockOffset + j));
        // }
        // AscendC::printf("\n");

        // AscendC::printf("============================== finally get selectBlockIdx: \n");
        // for(int j=selectBlockIdxLeft; j < selectBlockIdxRight; j++)
        // {
        //     AscendC::printf("%u, ", this->selectBlockIdxGm.GetValue(selectBlockOffset + j));
        // }
        // AscendC::printf("\n");
      ...
      // for (int64_t s2LoopCount = 0; s2LoopCount <= s2LoopLimit; s2LoopCount++) {
      int64_t s2LoopCount = 0;
      for (int64_t selectBlockIdx = selectBlockIdxLeft; selectBlockIdx < selectBlockIdxRight; selectBlockIdx++) {
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld selectLoop
        // begin.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);

        if (notLastTwoLoop) {
          s2LoopCount = this->selectBlockIdxGm.GetValue(selectBlockOffset + selectBlockIdx) - selectBlockIdxLeftValue;
        }
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld,
        // s2LoopCount:%ld.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx,s2LoopCount);

        if (taskId >= 1 && notLast) {
          // 对应extraInfo[(i+2)%3]
          WaitBmm1Result(extraInfo[(taskId + 2) % 3]);
        }
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld WaitBmm1Result
        // over.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);

        if (notLastTwoLoop) {
          this->SetExtraInfo(extraInfo[taskId % 3], taskId, s2LoopCount, s2LoopLimit, multiCoreInnerIdx, false);
          if constexpr (bmm1Format == CubeFormat::NZ) {
            if (extraInfo[taskId % 3].needNz2Nd == 1) {
              this->IterateBmm1(extraInfo[taskId % 3], this->bmm1Nz);
            } else {
        ...
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld IterateBmm1
        // over.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld ProcessVec1
        // over.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld WaitBmm2Result
        // over.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);
        ...
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld IterateBmm2
        // over.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);

        if (taskId > 1) {
          this->ProcessVec2(extraInfo[(taskId + 1) % 3]);
        }
        // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, selectBlockIdx:%ld ProcessVec2
        // over.\n",this->blockIdx, multiCoreInnerIdx, selectBlockIdx);

        taskId++;
      }
      // AscendC::printf("================  vec:%d,  multiCoreInnerIdx:%ld, S1Loop over.\n",this->blockIdx,
      // multiCoreInnerIdx);
    }
    // AscendC::printf("==== vec:%d, No enableL1Reuse over.\n",this->blockIdx);
  }
  // AscendC::printf("== vec:%d, Process over.\n",this->blockIdx);
}
...
__aicore__ inline void SparseAttentionScoreS1s2Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  enableL1Reuse>::IterateBmm1(SplitExtraInfo &extraInfo, matmul::Matmul<a1Type, b1Type, T2, bias1Type, MM_CFG> &bmm1) {
  if constexpr (layOutType != LayOutTypeEnum::LAYOUT_TND) {
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  for (int32_t loopIdx = 0; loopIdx < extraInfo.realSplitN; loopIdx++) {
    ...
    if (this->tilingData->inputParams.pseType != static_cast<uint32_t>(PseTypeEnum::PSE_OUTER_ADD_MUL_TYPE)) {
      pipe_barrier(PIPE_V);
    ...
    if constexpr (hasPse == true) {
      ...
      if constexpr (layOutType == LayOutTypeEnum::LAYOUT_TND) {
        if (this->tilingData->inputParams.sparseType == static_cast<uint8_t>(SparseModeEnum::BAND_LEFT_UP_CAUSAL) &&
            this->pseInfo.boIdx != 0) {
          innerAlibiFlag = true;
        }
      }

      if (this->pseInfo.pseType == static_cast<uint32_t>(PseTypeEnum::PSE_INNER_MUL_ADD_TYPE) ||
          this->pseInfo.pseType == static_cast<uint32_t>(PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE)) {
      ...
      pipe_barrier(PIPE_V);
      PseCompute<T, hasPse>(
        this->tilingData->inputParams.pseType !=
            static_cast<uint32_t>(PseTypeEnum::PSE_OUTER_ADD_MUL_TYPE)
        ? stage1PingTensor
        : actualUseTensor,
    ...
    if constexpr (hasDrop == true) {
      ...
      ComputeDropMask<T, hasDrop>(stage1PingTensor, stage1PingTensor, dropMaskUb, apiTmpBuffer, this->dropMaskInfo);
    }
    ...
    if constexpr (!IsSameType<T, INPUT_T>::value) {
      ...
      DataCopy(this->stage1Res[extraInfo.taskIdMod2][loopIdx * extraInfo.vec1S1BaseSize * extraInfo.s2AlignedSize],
  ...
  if constexpr (hasAtten == true) {
    ...
    if constexpr (layOutType == LayOutTypeEnum::LAYOUT_TND) {
      ...
      if (this->tilingData->inputParams.attenMaskCompressMode ==
        ...
        int64_t maskOffsetPre = ComputeOffsetForCausal(deltaPre, extraInfo.vec1S1BaseSize, this->s2BaseNratioSize,
        ...
        if ((extraInfo.s1Size + ((__gm__ int64_t *)this->prefixNAddr)[extraInfo.boIdx]) > extraInfo.s2Size) {
          ...
          this->attenMaskOffsetPre = ComputeOffsetForPrefixRectangle(deltaPre, this->s2BaseNratioSize,
                                                                     this->tilingData->inputParams.attenMaskS2Size);
    ...
    if (this->tilingData->inputParams.attenMaskCompressMode ==
      ...
      this->attenMaskOffsetPre = ComputeOffsetForCausal(deltaPre, extraInfo.vec1S1BaseSize, this->s2BaseNratioSize,
                                                        this->tilingData->inputParams.attenMaskS2Size);
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if constexpr (hasAtten == true) {
    ...
    if (this->tilingData->inputParams.attenMaskCompressMode == static_cast<uint8_t>(AttenMaskCompressMode::BAND_MODE)) {
    ...
    if (this->tilingData->inputParams.attenMaskCompressMode ==
      ...
      if (causalOrNextFactor >= 0 || preFactor >= 0 || deltaPre > extraInfo.s2Size) {
        ...
        int64_t intersectionX =
          extraInfo.s1Size - extraInfo.s2Size + ((__gm__ int64_t *)this->prefixNAddr)[extraInfo.boIdx];
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if constexpr (hasAtten == true) {
    ...
    int64_t gOffset = 0;
    int64_t s1Offset =
      extraInfo.s1oIdx * this->s1BaseSize * extraInfo.s2Size + loopIdx * extraInfo.vec1S1BaseSize * extraInfo.s2Size;
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if (likely(extraInfo.s2AlignedSize == extraInfo.s2RealSize)) {
    DataCopy2D(bmm1ResUb, this->mm1Res[extraInfo.taskIdMod2][loopIdx * extraInfo.vec1S1BaseSize * extraInfo.s2RealSize],
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if (unlikely(extraInfo.s2LoopCount == 0)) {
    if (IsBasicBlockInSoftMax(extraInfo.vec1S1RealSize, extraInfo.s2RealSize)) {
      SoftMaxTiling newTiling =
        AscendC::SoftMaxFlashV2TilingFuncImpl(extraInfo.vec1S1RealSize, extraInfo.s2AlignedSize, sizeof(T), sizeof(T),
                                              apiTmpBuffer.GetSize() / sizeof(T), false, true);
      SoftmaxFlashV2<T, false, true, true, false, SOFTMAX_DEFAULT_CFG>(srcTensor, sumUb, maxUb, srcTensor, expUb, sumUb,
                                                                       maxUb, apiTmpBuffer, newTiling);
    } else {
      SoftMaxTiling newTiling =
        AscendC::SoftMaxFlashV2TilingFuncImpl(extraInfo.vec1S1RealSize, extraInfo.s2AlignedSize, sizeof(T), sizeof(T),
                                              apiTmpBuffer.GetSize() / sizeof(T), false, false);
    ...
    if (IsBasicBlockInSoftMax(extraInfo.vec1S1RealSize, extraInfo.s2RealSize)) {
      ...
      SoftmaxFlashV2<T, true, true, true, false, SOFTMAX_DEFAULT_CFG>(srcTensor, sumUb, maxUb, srcTensor, expUb, sumUb,
                                                                      maxUb, apiTmpBuffer, newTiling);
    } else {
      SoftMaxTiling newTiling =
        AscendC::SoftMaxFlashV2TilingFuncImpl(extraInfo.vec1S1RealSize, extraInfo.s2AlignedSize, sizeof(T), sizeof(T),
                                              apiTmpBuffer.GetSize() / sizeof(T), true, false);
      SoftmaxFlashV2<T, true, true, false, false, SOFTMAX_DEFAULT_CFG>(srcTensor, sumUb, maxUb, srcTensor, expUb, sumUb,
                                                                       maxUb, apiTmpBuffer, newTiling);
    }
  }
  ...
  if (loopIdx == extraInfo.realSplitN - 1 && extraInfo.s2LoopCount == extraInfo.s2LoopLimit) {
    extraInfo.softmaxMaxOffset =
      (extraInfo.s1SizeAcc * this->n2G + extraInfo.n2oIdx * this->tilingData->inputParams.gSize * extraInfo.s1Size +
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if (likely(remain)) {
    Mul(bmm2ResUb[loop * repeatMaxSize], expUb[s1oIdx * extraInfo.vec2S1BaseSize * 8], bmm2ResUb[loop * repeatMaxSize],
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if constexpr (IsSameType<T, half>::value) {
    ...
    for (int i = 0; i < loop; i++) {
      Div(bmm2ResUb[i * repeatMaxSize], bmm2ResUb[i * repeatMaxSize], sumUb[s1oIdx * extraInfo.vec2S1BaseSize * 8],
          repeatMaxSize, extraInfo.vec2S1RealSize, repeatParams);
    }
    if (likely(remain)) {
...
SparseAttentionScoreS1s2Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if constexpr (layOutType == LayOutTypeEnum::LAYOUT_BSH) {
    ...
    dstStride =
      (this->tilingData->inputParams.n2Size * this->tilingData->inputParams.gSize - 1) * this->dSize * sizeof(INPUT_T);
  } else if constexpr (layOutType == LayOutTypeEnum::LAYOUT_SBH) {
...
#endif  // SPARSE_ATTENTION_SCORE_S1S2_BN2GS1_H
