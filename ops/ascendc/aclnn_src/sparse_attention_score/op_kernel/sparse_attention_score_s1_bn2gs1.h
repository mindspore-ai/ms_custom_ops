...
/*!
 * \file sparse_attention_score_s1_bn2gs1.h
 * \brief
 */

#ifndef SPARSE_ATTENTION_SCORE_S1_BN2GS1_H
#define SPARSE_ATTENTION_SCORE_S1_BN2GS1_H
...
class SparseAttentionScoreS1Bn2gs1 {
 public:
  __aicore__ inline SparseAttentionScoreS1Bn2gs1() {}
  ...
  using modeTypemm2 =
    typename AscendC::Conditional<(IsSameType<T, INPUT_T>::value == true),
 ...
 protected:
  ...
  __aicore__ inline void CopyToL1(const GlobalTensor<INPUT_T> &gmTensor, LocalTensor<INPUT_T> &localTensor, int64_t row,
                                  int64_t column, uint16_t srcDValue);
...
template <ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasPse, bool hasAtten, bool hasDrop, typename INPUT_T,
          ...
          bool enableL1Reuse>
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
  ...
  int64_t totalOffset =
    this->blockIdx * (mmNRatioOffset * 2 + stage1WorkspaceOffset + 2 * bmm2ResultOffset + pseAlibiOffset);
...
template <ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasPse, bool hasAtten, bool hasDrop, typename INPUT_T,
          typename T, bool isBasicBlock, CubeFormat bmm1Format, TPosition bmm2Source, CubeFormat bmm2SourceFormat,
          bool enableL1Reuse>
__aicore__ inline int64_t SparseAttentionScoreS1Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
...
SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
...
template <ImplModeEnum implMode, LayOutTypeEnum layOutType, bool hasPse, bool hasAtten, bool hasDrop, typename INPUT_T,
          typename T, bool isBasicBlock, CubeFormat bmm1Format, TPosition bmm2Source, CubeFormat bmm2SourceFormat,
          bool enableL1Reuse>
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
...
SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  if (this->tilingData->inputParams.sparseType == static_cast<uint8_t>(SparseModeEnum::CAUSAL)) {  // 下三角
    ...
    if (hasDrop && !this->tilingData->inputParams.needDropMaskOp) {
      this->s2StartIdx =
        Max(this->s1oIdx * this->tilingData->coreParams.s1BaseSize - this->tilingData->coreParams.s1SparseValidSize,
            0) /
        8 * 8;
    } else {
      this->s2StartIdx =
        Max(this->s1oIdx * this->tilingData->coreParams.s1BaseSize - this->tilingData->coreParams.s1SparseValidSize, 0);
    }
    ...
    this->s2StartIdx = 0;
    this->s2EndIdx =
      Max(s1BaseSize * (this->s1oIdx + 1) - this->tilingData->inputParams.s1Size + this->tilingData->inputParams.s2Size,
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
  enableL1Reuse>::LoopDivVec2(LocalTensor<T> &bmm2Res, LocalTensor<T> &sumTensor, int32_t s1OuterLoop,
                              int32_t s1OuterRemain, BinaryRepeatParams repeatParams) {
  ...
  for (int32_t i = 0; i < s1OuterLoop; ++i) {
    ...
    for (int32_t j = 0; j < innerLoop; ++j) {
      Div(bmm2ResDstTmp[j * this->repeatMaxSize], bmm2ResTmp[j * this->repeatMaxSize], sumTensor[i * softmaxSumMaxSize],
          this->repeatMaxSize, this->repeatMaxTimes, repeatParams);
    }
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
  ...
  if (likely(dstStride <= 65535)) {
    ...
    for (uint32_t i = 0; i < divCastCalcSize; ++i) {
      DataCopyPad(this->attentionOutGm[gmOffset + i * datacopyOffset], attentionOut[i * this->dSizeAlign16],
                  dataCopyParams);
    }
  }
}
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
  enableL1Reuse>::SetExtraInfo(SplitS1dExtraInfo &extraInfo, int64_t taskId, int64_t s2LoopCount, int64_t s2LoopLimit,
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
                                                    isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
                                                    enableL1Reuse>::ComputeBmm1Tail(SplitS1dExtraInfo &extraInfo) {
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
                                                    isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
                                                    enableL1Reuse>::Bmm1SetTensorA(SplitS1dExtraInfo &extraInfo) {
  // 计算gm上的offset
  int64_t bOffset = 0;
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
  ...
  nd2nzParams.srcDValue = srcDValue;  // 源操作数同一nd矩阵的相邻行起始地址间的偏移, 取值范围: [1, 65535]，单位: element
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
                                                    isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
                                                    enableL1Reuse>::SetBmm1TensorB(SplitS1dExtraInfo &extraInfo) {
  // 计算gm上的offset
  int64_t bOffset = 0;
  int64_t n2Offset = 0;
  int64_t s2Offset = 0;
  ...
  for (uint32_t loopIdx = 0; loopIdx < extraInfo.realSplitN; ++loopIdx) {
    ...
    if constexpr (!IsSameType<T, INPUT_T>::value) {
      if constexpr (bmm1Format == CubeFormat::NZ) {
        ...
        SetFlag<HardEvent::V_MTE3>(eventIdVToMte3);
        WaitFlag<HardEvent::V_MTE3>(eventIdVToMte3);
        DataCopy(this->stage1Res[extraInfo.taskIdMod2][loopIdx * extraInfo.s1BaseTimesS2Align16], stage1CastTensor,
                 extraInfo.vecS1TailSize * extraInfo.s2RealSizeAlign16);
        pipe_barrier(PIPE_V);
      }
    } else {
...
__aicore__ inline int64_t SparseAttentionScoreS1Bn2gs1<
  ...
  if constexpr (hasAtten == true) {
    ...
    if (this->tilingData->inputParams.attenMaskCompressMode ==
      ...
      delta = extraInfo.s1oIdx * s1BaseSize + loopIdx * extraInfo.vecS1BaseSize + this->tilingData->inputParams.s2Size -
              this->tilingData->inputParams.s1Size;
    } else if (this->tilingData->inputParams.attenMaskCompressMode ==
               static_cast<uint8_t>(AttenMaskCompressMode::BAND_MODE)) {
      int64_t deltaPre = extraInfo.s1oIdx * s1BaseSize + loopIdx * extraInfo.vecS1BaseSize - extraInfo.s2StartIdx -
                         this->tilingData->inputParams.preTokens - 1;
...
__aicore__ inline int64_t SparseAttentionScoreS1Bn2gs1<
  ...
  if constexpr (hasAtten == true) {
    ...
    int64_t gOffset = 0;
    int64_t s1Offset =
      extraInfo.s1oIdx * this->s1BaseS2 + loopIdx * extraInfo.vecS1BaseSize * this->tilingData->inputParams.s2Size;
...
__aicore__ inline void
SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
                             bmm2Source, bmm2SourceFormat, enableL1Reuse>::NdToNz(SplitS1dExtraInfo &extraInfo,
                                                                                  LocalTensor<INPUT_T> &nzResUb,
                                                                                  LocalTensor<INPUT_T> &vec1ResUb,
                                                                                  int64_t loopIdx) {
  ...
  for (int64_t outerIndex = 0; outerIndex < s1OuterLoop; ++outerIndex) {
    for (int64_t j = 0; j < s2InnerLoop; ++j) {
      Copy(nzResUbTmp[outerIndex * s1OuterTempOffset + j * offsetJ],
           vec1ResUbTmp[outerIndex * s1OuterBmm1Offset + j * 128], repeatMaxSize16, this->repeatMaxTimes, repeatParams);
    }
    if (s2InnerRemain) {
      Copy(nzResUbTmp[outerIndex * s1OuterTempOffset + s2InnerLoop * offsetJ],
           vec1ResUbTmp[outerIndex * s1OuterBmm1Offset + s2InnerLoop * 128], s2InnerRemain * 16, this->repeatMaxTimes,
           repeatParams);
    }
  }
  ...
  if (s1OuterRemain) {
    for (int64_t j = 0; j < s2InnerLoop; ++j) {
      Copy(nzResUbTmp[s1OuterLoop * s1OuterTempOffset + j * offsetJ],
           vec1ResUbTmp[s1OuterLoop * s1OuterBmm1Offset + j * 128], repeatMaxSize16, s1OuterRemain, repeatParams);
    }
    if (s2InnerRemain) {
      Copy(nzResUbTmp[s1OuterLoop * s1OuterTempOffset + s2InnerLoop * offsetJ],
           vec1ResUbTmp[s1OuterLoop * s1OuterBmm1Offset + s2InnerLoop * 128], s2InnerRemain * 16, s1OuterRemain,
           repeatParams);
    }
  }
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T,
                                                    isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
                                                    enableL1Reuse>::ComputeAttenMask(SplitS1dExtraInfo &extraInfo,
                                                                                     LocalTensor<T> &bmm1ResUb,
                                                                                     const uint8_t maskType) {
  if constexpr (hasAtten == true) {
    LocalTensor<uint8_t> attenMaskUb = this->maskTBufPing.template Get<uint8_t>();
...
SparseAttentionScoreS1Bn2gs1<implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format,
  ...
  uint32_t maxSumShape[] = {static_cast<uint32_t>(extraInfo.vecS1TailSize), static_cast<uint32_t>(this->fp32BaseSize)};
  LocalTensor<T> sumUb;
  LocalTensor<T> maxUb;
  ...
  if (IsBasicBlockInSoftMax(extraInfo.vecS1TailSize, extraInfo.s2RealSize)) {
    SoftMaxTiling newTiling =
      AscendC::SoftMaxFlashV2TilingFuncImpl(extraInfo.vecS1TailSize, extraInfo.s2RealSizeAlign16, sizeof(T), sizeof(T),
    ...
    SoftmaxFlashV2<T, false, true, false, false, SOFTMAX_DEFAULT_CFG>(srcTensor, sumUb, maxUb, srcTensor, expUb, sumUb,
                                                                      maxUb, apiTmpBuffer, newTiling);
  }
  ...
  if constexpr (implMode == ImplModeEnum::AA_INVALID_LINE_HIGH_PRECISION) {
    ...
    if (loopIdx == extraInfo.realSplitN - 1 || (loopIdx + 1) % extraInfo.softmaxCopyOutLimit == 0) {
      ...
      uint32_t currS1Size =
        static_cast<uint32_t>(Min(extraInfo.softmaxCopyOutS1Size, extraInfo.s1RealSize - extraInfo.softmaxOutOffset));
  ...
  if (loopIdx == extraInfo.realSplitN - 1 || (loopIdx + 1) % extraInfo.softmaxCopyOutLimit == 0) {
    ...
    int64_t calculateEleNum = Min(extraInfo.softmaxCopyOutS1Size, extraInfo.s1RealSize - extraInfo.softmaxOutOffset);
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
  enableL1Reuse>::WaitBmm2Result(matmul::Matmul<T2, b2Type, T3, bias2Type, MM_CFG> &bmm2) {
  bmm2.WaitIterateAll();
  bmm2.End();
}
...
__aicore__ inline void SparseAttentionScoreS1Bn2gs1<
  implMode, layOutType, hasPse, hasAtten, hasDrop, INPUT_T, T, isBasicBlock, bmm1Format, bmm2Source, bmm2SourceFormat,
  enableL1Reuse>::IterateBmm2(SplitS1dExtraInfo &extraInfo, matmul::Matmul<T2, b2Type, T3, bias2Type, MM_CFG> &bmm2) {
  int64_t bOffset = 0;
  int64_t n2Offset = 0;
  int64_t s2Offset = 0;
  ...
  if constexpr (bmm2Source == TPosition::TSCM) {
    ...
    if (extraInfo.taskId == 0 || n2Offset != bmm2N2OffsetLast || bOffset != bmm2BOffsetLast) {
      ...
      CopyToL1(this->valueGm[vCoreOffset], valueScmTensor, this->tilingData->inputParams.s2Size, dSize, srcDValue);
    }
  ...
  bmm2.SetTail(extraInfo.s1RealSize, this->tilingData->inputParams.dSize, extraInfo.s2RealSize);
  if constexpr (enableL1Reuse) {
    bmm2.template IterateAll<false>(this->mm2Res[extraInfo.taskIdMod2], false, false, true, extraInfo.lastNotPair);
  } else {
    bmm2.template IterateAll<false>(this->mm2Res[extraInfo.taskIdMod2], false, false, true);
  }
}

#endif  // SPARSE_ATTENTION_SCORE_S1_BN2GS1_H
