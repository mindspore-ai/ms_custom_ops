...
#include "unpad_fa_npd/op_host/unpad_fa_npd_tiling.h"
...
static ge::graphStatus GetKvHeadNum(gert::TilingContext *context, int32_t d, int32_t *kv_head_num) {
  ...
  FA_ASSERT_RET(context, k_shape != nullptr, "k shape is null", ge::GRAPH_FAILED);
...
static ge::graphStatus UnpadFaNpdCheck(gert::TilingContext *context, const tiling_info &ti) {
  FA_ASSERT_RET(context, ti.head_num > 0, "head_num is invalid", ge::GRAPH_FAILED);
  FA_ASSERT_RET(
    context,
    (ti.kv_head_num > 0 && ti.kv_head_num <= ti.head_num && ti.head_num % ti.kv_head_num == 0) || (ti.kv_head_num == 0),
    "kvHead is invalid", ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.q_seq_len.size() == ti.kv_seq_len.size(), "qSeqLen size Not equal to kvSeqLen",
                ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.q_seq_len.data() != nullptr, "qSeq cannot be nullptr", ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.kv_seq_len.data() != nullptr, "kvSeq cannot be nullptr", ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.batch > 0 && ti.batch <= KMaxBatch, "batch is invalid", ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, static_cast<int32_t>(ti.q_seq_len.size()) == ti.batch, "SeqLen size is invalid",
                ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.is_triu == 0 || ti.is_triu == 1, "isTriuMask is invalid", ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.is_long_seq == 0 || ti.is_long_seq == 1, "isLongSeq is invalid", ge::GRAPH_FAILED);
  FA_ASSERT_RET(context, ti.d <= kMaxEmbedSize && ti.d > 0, "headdim is invalid", ge::GRAPH_FAILED);
  return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InitUnpadFaNpdTilingInfo(gert::TilingContext *context, tiling_info *ti) {
  ti->head_num = *context->GetAttrs()->GetAttrPointer<int64_t>(0);
  ti->tor = static_cast<float>(*context->GetAttrs()->GetAttrPointer<float>(1));
  ti->npd = std::string(context->GetAttrs()->GetAttrPointer<char>(3)) == std::string("NPD");
  ti->page_size = *context->GetAttrs()->GetAttrPointer<int64_t>(4);
  ti->splitm = false;
  FA_ASSERT_RET(context, (GetD(context, ti->head_num, &ti->d) == ge::GRAPH_SUCCESS), "fail to get embed size",
                ge::GRAPH_FAILED);
  ti->d_round = UpRound(ti->d);
  FA_ASSERT_RET(context, (GetKvHeadNum(context, ti->d, &ti->kv_head_num) == ge::GRAPH_SUCCESS),
                "fail to get embed size", ge::GRAPH_FAILED);
...
static uint32_t ComputeBatchTilingParams(gert::TilingContext *context, const tiling_info &ti,
                                         UnpadFaNpdTilingData *tiling) {
  uint64_t in_out_offset = 0;
  uint64_t kv_offset = 0;
  uint32_t q_blocks_total = 0;
  ...
  for (int i = 0; i < ti.batch; i++) {
    ...
    auto tiling_k = ti.d_round < triu_dim ? triu_dim : ti.d_round;
    auto chunk_size = cur_kv_seq - cur_q_seq;
    ...
    uint32_t mUbd =
      std::min((pp_buff_size / std::max(ti.d_round, pp_nn[nIbd]) / block_size) * block_size, cur_q_seq_round);
    ...
    kv_offset += static_cast<uint64_t>(UpRound(cur_kv_seq, ti.page_size) * ti.kv_head_num * ti.d);
  }
...
static ge::graphStatus UnpadFaNpdTiling(gert::TilingContext *context) {
  ...
  FA_ASSERT_RET(context, InitUnpadFaNpdTilingInfo(context, &ti) == ge::GRAPH_SUCCESS,
                "fail to initialize flash attention tiling info", ge::GRAPH_FAILED);
  ...
  auto q_blocks_total = ComputeBatchTilingParams(context, ti, &tiling);
  ...
  tiling_head[KTorIdx] = FloatToBits(ti.tor);
  ...
  tiling_head[KKvLayoutIdx] = ti.npd ? kDim2 : 0;
  tiling_head[KPageSizeIdx] = ti.page_size;
  ...
  tiling_head[KWorkspaceIdx] = (static_cast<uint32_t>(work_size));
...
IMPL_OP_OPTILING(UnpadFaNpd).Tiling(UnpadFaNpdTiling);
}  // namespace optiling
