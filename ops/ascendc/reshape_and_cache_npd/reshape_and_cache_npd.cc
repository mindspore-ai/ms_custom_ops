...
#include <utility>
#include "op_host/reshape_and_cache_npd.h"
...
static mindspore::ShapeVector ReshapeAndCacheNpdDoInferShape(const mindspore::ShapeVector &shape,
                                                             ...
                                                             const bool &key_value_layout_npd) {
  ShapeVector out_shape = shape;
  if (key_value_layout_npd) {
    ...
    if (out_shape.at(0) >= 0) {
      ...
      auto max_seq = shape.at(0) + (block_size - 1) * batch;
...
static void ReshapeAndCacheNpdCheckLayout(const std::string &op_name, const std::string &out_kv_cache_layout,
  ...
  if (out_kv_cache_layout != ND_LAYOUT && out_kv_cache_layout != NPD_LAYOUT) {
    MS_LOG(EXCEPTION) << op_name << ", out kv cache layout must be ND or NPD, but got" << out_kv_cache_layout;
  }
...
class OPS_API ReshapeAndCacheNpdOpFuncImpl : public OpFuncImpl {
 public:
  ShapeArray InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const override {
    ...
    auto blk_tbl_shape =
      input_infos[static_cast<size_t>(ReshapeAndCacheNpdInputIndex::kInputBlockTblIndex)]->GetShape();
    if (blk_tbl_shape.size() != kDim2) {
      MS_LOG(EXCEPTION) << "The dim of block table shape is illegal, "
                        << "block_tbl_shape.dim=" << blk_tbl_shape.size();
    }
    int32_t batch = blk_tbl_shape.at(0);
    ...
    ShapeVector out_shape = ReshapeAndCacheNpdDoInferShape(key_shape, k_cache_shape, batch, cache_kv_layout, kv_layout);
...
class ReshapeAndCacheNpdAscend : public AclnnCustomKernelMod {
 ...
 private:
  ...
  std::string kv_cache_mode_ = NPD_LAYOUT;
...
std::vector<ms::Tensor> reshape_and_cache_npd_custom(const ms::Tensor &key, const ms::Tensor &value,
  ...
  auto cache_layout = kv_cache_layout == std::string(NPD_LAYOUT);
  auto kv_layout = key_value_layout == std::string(NPD_LAYOUT);

  ShapeVector out_shape =
    ReshapeAndCacheNpdDoInferShape(key.shape(), key_cache.shape(), block_tbl.shape().at(0), cache_layout, kv_layout);
...
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("reshape_and_cache_npd", &ms_custom_ops::reshape_and_cache_npd_custom, pybind11::arg("key"),
        ...
        pybind11::arg("kv_cache_layout") = NPD_LAYOUT, pybind11::arg("key_value_layout") = NPD_LAYOUT);
}
