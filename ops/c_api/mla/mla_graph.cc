...
#include "ops/c_api/mla/mla_common.h"
...
static void CheckParam(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) {
  ...
  MS_CHECK_VALUE(kv_heads == 1,
                 CheckAndConvertUtils::FormatCommMsg("For MLA The kv_head_num must be 1 , but got : ", kv_heads));

  auto q_heads = input_infos[kMlaInputNumHeadIndex]->GetScalarValueWithCheck<int64_t>();
  MS_CHECK_VALUE(q_heads <= kMLAQheadMax, CheckAndConvertUtils::FormatCommMsg(
                                            "For MLA The head_num must be <= ", kMLAQheadMax, ", but got : ", q_heads));
  MS_CHECK_VALUE(ALIGN_16(q_heads), CheckAndConvertUtils::FormatCommMsg(
                                      "For MLA The head_num must be the multiple of 16, but got : ", q_heads));
...
class Mla : public InternalKernelMod {
 ...
 protected:
  internal_v2::InternalOpPtr CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
    ...
    param_.type = internal_v2::MLAParam::kSplitCache;
    ...
    param_.mask_type =
      static_cast<internal_v2::MLAParam::MaskType>(ms_inputs[kMlaInputMaskTypeIndex]->GetValueWithCheck<int64_t>());
    ...
    if (input_format == kKVFormatNZ) {
      ...
      inputs_new[kMlaInputKvCacheIndex].SetFormat(internal_v2::kFormatFRACTAL_NZ);
      inputs_new[kMlaInputKropeIndex].SetFormat(internal_v2::kFormatFRACTAL_NZ);
      return internal_v2::CreateMLAOp(inputs_new, outputs, param_, internal_v2::kInternalMLAOpName);
    }
