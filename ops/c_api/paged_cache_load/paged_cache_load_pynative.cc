...
#include "ops/c_api/paged_cache_load/paged_cache_load_common.h"
...
class PagedCacheLoadRunner : public InternalPyboostRunner {
 public:
  ...
  internal_v2::PagedCacheLoadParam param_;
...
std::vector<ms::Tensor> npu_paged_cache_load(const ms::Tensor &key_cache, const ms::Tensor &value_cache,
                                             const ms::Tensor &block_table, const ms::Tensor &seq_lens,
  ...
  if (seq_lens.data_type() != mindspore::TypeId::kNumberTypeInt32) {
    MS_LOG(EXCEPTION) << "For " << op_name << ", the seq_lens dtype must be int32, but got : " << seq_lens.data_type();
  }
...
auto pyboost_paged_cache_load(const ms::Tensor &key_cache, const ms::Tensor &value_cache, const ms::Tensor &block_table,
                              const ms::Tensor &seq_lens, const std::optional<ms::Tensor> &seq_starts,
                              std::optional<int64_t> kv_cache_cfg, std::optional<bool> is_seq_lens_cumsum_type,
                              std::optional<bool> has_seq_starts) {
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::kNumber2>(npu_paged_cache_load, key_cache, value_cache,
                                                                    block_table, seq_lens, seq_starts, kv_cache_cfg,
                                                                    is_seq_lens_cumsum_type, has_seq_starts);
}
}  // namespace ms_custom_ops

MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("paged_cache_load", &ms_custom_ops::pyboost_paged_cache_load, "Paged Cache Load", pybind11::arg("key_cache"),
        pybind11::arg("value_cache"), pybind11::arg("block_table"), pybind11::arg("seq_lens"),
        pybind11::arg("seq_starts") = std::nullopt, pybind11::arg("kv_cache_cfg") = 0,
        pybind11::arg("is_seq_lens_cumsum_type") = false, pybind11::arg("has_seq_starts") = false);
}
