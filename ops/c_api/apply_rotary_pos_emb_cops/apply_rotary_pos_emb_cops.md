# apply_rotary_pos_emb_cops算子

## 描述

apply_rotary_pos_emb_cops算子用于计算旋转编码操作。该算子底层调用的是aclnnApplyRotaryPosEmbV2算子。

## 输入参数

| Name                | DType           | Shape                                  | Optional | Inplace | Format | Description                                            |
|---------------------|-----------------|----------------------------------------|----------|---------|--------|--------------------------------------------------------|
| query               | Tensor          | 4维[batch_size, seq_len, q_head_num, head_dim] | No       | Yes      | ND     | 执行旋转位置编码的第一个变量，同时作为输出。                              |
| key                 | Tensor          | 4维[batch_size, seq_len, k_head_num, head_dim] | No       | Yes      | ND     | 执行旋转位置编码的第二个变量，同时作为输出。                              |
...
@ms.jit
def apply_rotary_pos_emb_cops_func(query, key, cos, sin, layout="BSND", rotary_mode="half"):
   return ms_custom_ops.apply_rotary_pos_emb_cops(query, key, cos, sin, layout, rotary_mode)
...
apply_rotary_pos_emb_cops_func(query, key, cos, sin)
```
