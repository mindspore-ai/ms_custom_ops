 ...
 public:
  ...
  void GetWorkSpaceInfo(const std::vector<KernelTensor *> &inputs,
    ...
    if (x2_format == "FRACTAL_NZ") {
      ...
      auto storage_info = GetNZFormatStorageInfo(nd_shape, x2_tensor_->dtype_id());
...
ms::Tensor quant_batch_matmul_custom(const ms::Tensor &x1, const ms::Tensor &x2, const ms::Tensor &scale,
  ...
  auto output_shape = BatchMatMulMakeShape(x1_shape, x2_shape, transpose_x1, transpose_x2, kQbmmMatSize);
...
auto pyboost_quant_batch_matmul(const ms::Tensor &x1, const ms::Tensor &x2, const ms::Tensor &scale,
                                const std::optional<ms::Tensor> &offset, const std::optional<ms::Tensor> &bias,
                                const std::optional<ms::Tensor> &pertoken_scale, bool transpose_x1, bool transpose_x2,
                                const std::string &x2_format, const int64_t output_dtype) {
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::kNumber1>(ms_custom_ops::quant_batch_matmul_custom, x1, x2,
                                                                    scale, offset, bias, pertoken_scale, transpose_x1,
                                                                    transpose_x2, x2_format, output_dtype);
}

constexpr int64_t kFloat16 = 42;
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("quant_batch_matmul", &pyboost_quant_batch_matmul, "quant_batch_matmul", pybind11::arg("x1"),
        pybind11::arg("x2"), pybind11::arg("scale"), pybind11::arg("offset") = std::nullopt,
        pybind11::arg("bias") = std::nullopt, pybind11::arg("pertoken_scale") = std::nullopt,
        pybind11::arg("transpose_x1") = false, pybind11::arg("transpose_x2") = false, pybind11::arg("x2_format") = "ND",
        pybind11::arg("output_dtype") = kFloat16);
}
