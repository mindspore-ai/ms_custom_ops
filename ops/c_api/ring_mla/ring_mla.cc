...
#include "ops/c_api/ring_mla/ring_mla.h"
...
ShapeArray CustomRingMLAOpFuncImpl::InferShape(const PrimitivePtr &primitive,
                                               const InferInfoPtrList &input_infos) const {
  auto calc_type =
    static_cast<internal_v2::RingMLAParam::CalcType>(input_infos[kCalcTypeIdx]->GetScalarValueWithCheck<int64_t>());
  is_input_softmax_lse_ = (calc_type == internal_v2::RingMLAParam::CalcType::CALC_TYPE_DEFAULT);
...
bool CustomRingMLA::RingMLAParamCheck(const internal_v2::RingMLAParam &op_param) {
  if (op_param.calcType != internal_v2::RingMLAParam::CalcType::CALC_TYPE_DEFAULT &&
      op_param.calcType != internal_v2::RingMLAParam::CalcType::CALC_TYPE_FISRT_RING) {
  ...
  if (op_param.kvHeadNum > 0 && op_param.headNum % op_param.kvHeadNum != 0) {
    MS_LOG(ERROR) << "Ring MLA expects headNum to be divisible by kvHeadNum, "
                  << "But got param.headNum = " << op_param.headNum << ", param.kvHeadNum = " << op_param.kvHeadNum;
    return false;
  }
  ...
  if (op_param.maskType != internal_v2::RingMLAParam::MaskType::NO_MASK &&
      op_param.maskType != internal_v2::RingMLAParam::MaskType::MASK_TYPE_TRIU) {
    MS_LOG(ERROR) << "Ring MLA expects maskType as one of NO_MASK, MASK_TYPE_TRIU, "
                  << "But got param.maskType = " << op_param.maskType;
    return false;
  }
  if (op_param.inputLayout != internal_v2::RingMLAParam::InputLayout::TYPE_BSND) {
    MS_LOG(ERROR) << "Ring MLA only supports inputLayout as TYPE_BSND, "
                  << "But got param.inputLayout = " << op_param.inputLayout;
    return false;
  }
  if (op_param.kernelType != internal_v2::RingMLAParam::KernelType::KERNELTYPE_HIGH_PRECISION) {
...
static void ExtractSeqLenVector(KernelTensor *const seq_len_tensor, std::vector<int32_t> *out_vec) {
  ...
  if (dtype == kNumberTypeInt64) {
    ...
    *out_vec = seq_len_tensor->GetValueWithCheck<std::vector<int32_t>>();
    ...
    MS_LOG(EXCEPTION) << "actual_seq_lengths data type must be Int32 or Int64, but got " << TypeIdToString(dtype);
  }
}
...
static bool GetSeqLenFromInputAndCheckUpdate(const std::string &kernel_name, const std::string &tensor_name,
  ...
  MS_LOG(INFO) << "For op '" << kernel_name << "', param seq_len must be set, but none of '" << tensor_name
               << "' is found in tensor_input";
  if (seq_len->empty()) {
...
internal_v2::InternalOpPtr CustomRingMLA::CreateKernel(const internal_v2::InputsImmutableInfoList &inputs_ii,
                                                       const internal_v2::OutputsImmutableInfoList &outputs_ii,
  ...
  param_.maskType =
    static_cast<internal_v2::RingMLAParam::MaskType>(ms_inputs[kMaskTypeIdx]->GetValueWithCheck<int64_t>());
  param_.calcType =
    static_cast<internal_v2::RingMLAParam::CalcType>(ms_inputs[kCalcTypeIdx]->GetValueWithCheck<int64_t>());
  ...
  (void)GetSeqLenFromInputAndCheckUpdate(kernel_name_, "batch_valid_length", ms_inputs[kKVSeqLenIdx], &param_.kvSeqLen);
  ...
  created_flag_ = true;
  return internal_v2::CreateRingMLAOp(inputs_ii, outputs_ii, param_, internal_v2::kInternalRingMLAOpName);
}

bool CustomRingMLA::UpdateParam(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) {
  if (created_flag_) {
  ...
  bool q_need_update =
    GetSeqLenFromInputAndCheckUpdate(kernel_name_, "q_seq_lens", inputs[kQSeqLenIdx], &param_.qSeqLen);
  bool kv_need_update =
    GetSeqLenFromInputAndCheckUpdate(kernel_name_, "batch_valid_length", inputs[kKVSeqLenIdx], &param_.kvSeqLen);
...
void CustomRingMLA::InitKernelInputsOutputsIndex() {
  kernel_inputs_index_ = {kQueryIdx,    kQueryRopeIdx,  kKeyIdx,   kKeyRopeIdx,  kValueIdx,
                          kMaskIdx,     kAlibiCoeffIdx, kDeqQKIdx, kOffsetQKIdx, kDeqPVIdx,
                          kOffsetPVIdx, kQuantPIdx,     kLogNIdx,  kOPrevIdx,    kLsePrevIdx};
