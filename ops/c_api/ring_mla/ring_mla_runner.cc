...
#include "ops/c_api/ring_mla/ring_mla_runner.h"
...
void RingMLARunner::SetRingMLAParam(int64_t head_num, float scale_value, int64_t kv_head_num, int64_t mask_type,
  ...
  param_.maskType = static_cast<internal_v2::RingMLAParam::MaskType>(mask_type);
  param_.calcType = static_cast<internal_v2::RingMLAParam::CalcType>(calc_type);
}
...
internal_v2::InternalOpPtr RingMLARunner::CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
                                                       const internal_v2::OutputsImmutableInfoList &outputs) {
  created_flag_ = true;
  return internal_v2::CreateRingMLAOp(inputs, outputs, param_, internal_v2::kInternalRingMLAOpName);
}
...
ms::Tensor GenLseOutTensor(const ms::Tensor &query, const std::optional<ms::Tensor> &lse_prev,
  ...
  using CalcType = internal_v2::RingMLAParam::CalcType;
...
auto pyboost_ring_mla(const ms::Tensor &query, const ms::Tensor &query_rope, const ms::Tensor &key,
  ...
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::kNumber2>(
    ms_custom_ops::npu_ring_mla, query, query_rope, key, key_rope, value, mask, alibi_coeff, deq_scale_qk,
    deq_offset_qk, deq_scale_pv, deq_offset_pv, quant_p, log_n, o_prev, lse_prev, q_seq_lens, context_lens, head_num,
    scale_value, kv_head_num, mask_type, calc_type);
}
...
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("ring_mla", &pyboost_ring_mla, "Ring MLA", pybind11::arg("query"), pybind11::arg("query_rope"),
        ...
        pybind11::arg("lse_prev") = std::nullopt, pybind11::arg("q_seq_lens") = std::nullopt,
        pybind11::arg("context_lens") = std::nullopt, pybind11::arg("head_num") = 0, pybind11::arg("scale_value") = 1.0,
        pybind11::arg("kv_head_num") = 0, pybind11::arg("mask_type") = 0, pybind11::arg("calc_type") = 0);
}
