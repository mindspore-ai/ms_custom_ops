...
class CustomFusedAddTopkDiv : public InternalKernelMod {
 ...
 protected:
  ...
  internal_v2::InternalOpPtr CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
    ...
    internal_v2::FusedAddTopkDivParam param;
    ...
    return internal_v2::CreateFusedAddTopkDivOp(inputs, outputs, param, internal_v2::kInternalFusedAddTopkDivOpName);
  }
};
}  // namespace ms_custom_ops
...
class FusedAddTopkDivRunner : public InternalPyboostRunner {
 ...
 protected:
  internal_v2::InternalOpPtr CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
    ...
    return internal_v2::CreateFusedAddTopkDivOp(inputs, outputs, param_, internal_v2::kInternalFusedAddTopkDivOpName);
  }

 private:
  internal_v2::FusedAddTopkDivParam param_;
};
...
std::vector<ms::Tensor> npu_fused_add_topk_div(const ms::Tensor &x, const ms::Tensor &add_num, int64_t group_num,
  ...
  auto runner = std::make_shared<FusedAddTopkDivRunner>(op_name);
...
auto pyboost_fused_add_topk_div(const ms::Tensor &x, const ms::Tensor &add_num, int64_t group_num, int64_t group_topk,
  ...
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::FusedAddTopKDivOutputIndex::kFusedAddTopKDivOutputNums>(
