 ...
 public:
  ShapeArray InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const override {
    ...
    MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
      (kv_shape.size() == static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize4)),
      op_name + " kv input size should be " +
        std::to_string(static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize4)) + ", but now got " +
        std::to_string(kv_shape.size()));

    MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
      (gamma_shape.size() == static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize1)),
      op_name + " gamma input size should be " +
    ...
    MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
      ((cos_shape.size() == static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize4))),
      op_name + std::to_string(static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize4)) +
        ", but now got cos " + std::to_string(cos_shape.size()));

    MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
      ((sin_shape.size() == static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize4))),
      op_name + std::to_string(static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize4)) +
        ", but now got sin " + std::to_string(sin_shape.size()));
    ...
    if (cache_mode == KvRmsNormRopeCacheMode::kKvRmsNormRopeCacheModeNorm) {
      MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
        (index_shape.size() == static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize2)),
        op_name + " index input size should be " +
      ...
      MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
        (index_shape.size() == static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize1)),
        op_name + " index input size should be " +
          std::to_string(static_cast<size_t>(KvRmsNormRopeCacheSize::kKvRmsNormRopeCacheSize1)) + ", but now got" +
          std::to_string(index_shape.size()));
    }
    ...
    MS_CUSTOM_OPS_EXCEPTION_IF_CHECK_FAIL(
      ...
      op_name + " kCache or CKvCache input size should be " +
