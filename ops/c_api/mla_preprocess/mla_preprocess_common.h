...
inline internal_v2::InternalOpPtr CreateMlaPreprocessOpWithFormat(const internal_v2::InputsImmutableInfoList &inputs,
                                                                  const internal_v2::OutputsImmutableInfoList &outputs,
                                                                  const internal_v2::MlaPreprocessParam &param) {
  auto inputs_clone = inputs;
  inputs_clone[kMlaPreprocessWdqkvIndex].SetFormat(internal_v2::kFormatFRACTAL_NZ);
  inputs_clone[kMlaPreprocessWuqIndex].SetFormat(internal_v2::kFormatFRACTAL_NZ);
  if (param.cache_mode == kMlaPreCacheModeQKSplitQuant || param.cache_mode == kMlaPreCacheModeQKSplitNz) {
    inputs_clone[kMlaPreprocessKeyCacheIndex].SetFormat(internal_v2::kFormatFRACTAL_NZ);
    inputs_clone[kMlaPreprocessKropeCacheIndex].SetFormat(internal_v2::kFormatFRACTAL_NZ);
  }
  return internal_v2::CreateMlaPreprocessOp(inputs_clone, outputs, param, internal_v2::kInternalMlaPreprocessOpName);
};
