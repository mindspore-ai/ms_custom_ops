...
inline internal_v2::InternalOpPtr CreateReshapeAndCacheOpWithFormat(
  const internal_v2::InputsImmutableInfoList &inputs, const internal_v2::OutputsImmutableInfoList &outputs,
  const internal_v2::ReshapeAndCacheParam &param, int32_t cache_mode) {
  if (cache_mode == static_cast<int32_t>(CacheMode::NZ)) {
    auto inputs_clone = inputs;
    inputs_clone[static_cast<size_t>(InputIndex::kInputKeyCacheIndex)].SetFormat(internal_v2::kFormatFRACTAL_NZ);
    inputs_clone[static_cast<size_t>(InputIndex::kInputValueCacheIndex)].SetFormat(internal_v2::kFormatFRACTAL_NZ);
    return internal_v2::CreateAsdReshapeAndCacheOp(inputs_clone, outputs, param,
                                                   internal_v2::kInternalAsdReshapeAndCacheOpName);
  }
  return internal_v2::CreateAsdReshapeAndCacheOp(inputs, outputs, param,
                                                 internal_v2::kInternalAsdReshapeAndCacheOpName);
}
...
class CustomReshapeAndCache : public InternalKernelMod {
 ...
 protected:
  internal_v2::InternalOpPtr CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
    ...
    internal_v2::ReshapeAndCacheParam param;
...
auto pyboost_reshape_and_cache(const ms::Tensor &key, const std::optional<ms::Tensor> &value,
  ...
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::kNumber0>(
    ms_custom_ops::npu_reshape_and_cache, key, value, key_cache, value_cache, slot_mapping, cache_mode, head_num);
}
...
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("reshape_and_cache", &pyboost_reshape_and_cache, "Reshape And Cache", pybind11::arg("key"),
        ...
        pybind11::arg("cache_mode") = 0, pybind11::arg("head_num") = 0);
}
