...
constexpr uint32_t ROPE_QK_SUPPORT_DIM1 = 2;
constexpr uint32_t ROPE_QK_SUPPORT_DIM2 = 4;
constexpr uint32_t ROPE_COS_SIN_SUPPORT_DIM = 2;
constexpr uint32_t ROPE_SEQLEN_SUPPORT_DIM = 1;
constexpr uint32_t ROPE_ROTARY_COEFF_DEFAULT = 4;
enum class RopeInputIndex : size_t {
  kRopeQueryIndex = 0,
  kRopeKeyIndex,
  kRopeCosIndex,
  kRopeSinIndex,
  kRopeSeqLenIndex,
  kRopeRotaryCoeffIndex,
  kRopeCosFormatIndex,
  kRopeInputsNum,
};

enum class RopeOutputIndex : size_t {
  kRopeQueryIndex = 0,
  kRopeKeyIndex,
  kRopeOutputsNum,
};

class OPS_API RopeOpFuncImpl : public OpFuncImpl {
 public:
  ShapeArray InferShape(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const override {
    if (input_infos[static_cast<size_t>(RopeInputIndex::kRopeQueryIndex)]->IsDynamicRank() ||
        input_infos[static_cast<size_t>(RopeInputIndex::kRopeKeyIndex)]->IsDynamicRank() ||
        input_infos[static_cast<size_t>(RopeInputIndex::kRopeCosIndex)]->IsDynamicRank() ||
        input_infos[static_cast<size_t>(RopeInputIndex::kRopeSinIndex)]->IsDynamicRank() ||
        input_infos[static_cast<size_t>(RopeInputIndex::kRopeSeqLenIndex)]->IsDynamicRank()) {
      return {input_infos[static_cast<size_t>(RopeInputIndex::kRopeQueryIndex)]->GetShape(),
              input_infos[static_cast<size_t>(RopeInputIndex::kRopeKeyIndex)]->GetShape()};
    }
    MS_EXCEPTION_IF_NULL(primitive);
    auto op_name = primitive->name();
    auto query_shape = input_infos[static_cast<size_t>(RopeInputIndex::kRopeQueryIndex)]->GetShape();
    auto key_shape = input_infos[static_cast<size_t>(RopeInputIndex::kRopeKeyIndex)]->GetShape();
    auto cos_shape = input_infos[static_cast<size_t>(RopeInputIndex::kRopeCosIndex)]->GetShape();
    auto sin_shape = input_infos[static_cast<size_t>(RopeInputIndex::kRopeSinIndex)]->GetShape();
    auto seqlen_shape = input_infos[static_cast<size_t>(RopeInputIndex::kRopeSeqLenIndex)]->GetShape();
    ...
    if (query_shape.size() != ROPE_QK_SUPPORT_DIM1 && query_shape.size() != ROPE_QK_SUPPORT_DIM2 ||
        cos_shape.size() != ROPE_COS_SIN_SUPPORT_DIM || seqlen_shape.size() != ROPE_SEQLEN_SUPPORT_DIM) {
      MS_LOG(EXCEPTION) << op_name
  ...
  std::vector<TypeId> InferType(const PrimitivePtr &primitive, const InferInfoPtrList &input_infos) const override {
    MS_EXCEPTION_IF_NULL(primitive);
    auto op_name = primitive->name();
    auto query_type = input_infos[static_cast<size_t>(RopeInputIndex::kRopeQueryIndex)]->GetType();
    auto key_type = input_infos[static_cast<size_t>(RopeInputIndex::kRopeKeyIndex)]->GetType();
    auto cos_type = input_infos[static_cast<size_t>(RopeInputIndex::kRopeCosIndex)]->GetType();
    auto sin_type = input_infos[static_cast<size_t>(RopeInputIndex::kRopeSinIndex)]->GetType();
...
class Rope : public InternalKernelMod {
 public:
  Rope() : InternalKernelMod() {}
  ~Rope() = default;

  void InitKernelInputsOutputsIndex() override {
    kernel_inputs_index_ = {
      static_cast<size_t>(RopeInputIndex::kRopeQueryIndex), static_cast<size_t>(RopeInputIndex::kRopeKeyIndex),
      static_cast<size_t>(RopeInputIndex::kRopeCosIndex), static_cast<size_t>(RopeInputIndex::kRopeSinIndex),
      static_cast<size_t>(RopeInputIndex::kRopeSeqLenIndex)};
    kernel_outputs_index_ = {static_cast<size_t>(RopeOutputIndex::kRopeQueryIndex),
                             static_cast<size_t>(RopeOutputIndex::kRopeKeyIndex)};
  }
 ...
 protected:
  internal_v2::InternalOpPtr CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
    ...
    param.rotary_coeff = static_cast<int32_t>(
      ms_inputs.at(static_cast<size_t>(RopeInputIndex::kRopeRotaryCoeffIndex))->GetValue<int64_t>().value());
    param.cos_format = static_cast<int32_t>(
      ms_inputs.at(static_cast<size_t>(RopeInputIndex::kRopeCosFormatIndex))->GetValue<int64_t>().value());
    return internal_v2::CreateRopeOp(inputs, outputs, param, internal_v2::kInternalRopeOpName);
  }
};
}  // namespace ms_custom_ops

REG_GRAPH_MODE_OP(rope, ms_custom_ops::RopeOpFuncImpl, ms_custom_ops::Rope);
...
namespace ms_custom_ops {
class RopeRunner : public InternalPyboostRunner {
 public:
 ...
 private:
  int32_t rotary_coeff_{ROPE_ROTARY_COEFF_DEFAULT};
...
std::vector<ms::Tensor> npu_rope(const ms::Tensor &query, const ms::Tensor &key, const ms::Tensor &cos,
                                 ...
                                 int64_t cos_format) {
  auto op_name = "Rope";
  auto runner = std::make_shared<RopeRunner>(op_name);
...
auto pyboost_rope(const ms::Tensor &query, const ms::Tensor &key, const ms::Tensor &cos, const ms::Tensor &sin,
                  const ms::Tensor &seq_len, int64_t rotary_coeff, int64_t cos_format) {
...
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("rope", &pyboost_rope, "Rope", pybind11::arg("query"), pybind11::arg("key"), pybind11::arg("cos"),
        pybind11::arg("sin"), pybind11::arg("seq_len"), pybind11::arg("rotary_coeff") = 4,
        pybind11::arg("cos_format") = 0);
}
