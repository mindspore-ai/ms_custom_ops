...
void CheckTypeValid(TypeId input_type, TypeId output_type) {
  ...
  if (!valid_type.count(input_type) || !valid_type.count(output_type)) {
    MS_LOG(EXCEPTION) << "For 'type_cast'"
                      ...
                      << TypeIdToString(input_type) << ", output type: " << TypeIdToString(output_type);
  }
}
...
class CustomTypeCast : public InternalKernelMod {
 public:
  ...
  bool Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
    ...
    auto ret = CALL_ASCEND_API(aclrtMemcpyAsync, outputs[0]->device_ptr(), copy_size, inputs[0]->device_ptr(),
                               copy_size, ACL_MEMCPY_DEVICE_TO_DEVICE, stream_ptr);
    if (ret != ACL_SUCCESS) {
...
REG_GRAPH_MODE_OP(type_cast, ms_custom_ops::CustomTypeCastOpFuncImpl, ms_custom_ops::CustomTypeCast);
...
class TypeCastRunner : public InternalPyboostRunner {
 public:
  ...
  void LaunchKernel() override {
    ...
    auto ret = CALL_ASCEND_API(aclrtMemcpyAsync, outputs[0].GetDataPtr(), copy_size, inputs[0].GetDataPtr(), copy_size,
                               ACL_MEMCPY_DEVICE_TO_DEVICE, this->stream());
    if (ret != ACL_SUCCESS) {
      MS_LOG(ERROR) << "For 'TypeCast', Memcpy failed, ret=" << ret;
    }
...
ms::Tensor npu_type_cast(const ms::Tensor &x, int64_t output_dtype) {
  ...
  auto runner = std::make_shared<TypeCastRunner>(op_name);
...
auto pyboost_type_cast(const ms::Tensor &x, int64_t output_dtype) {
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::kNumber1>(ms_custom_ops::npu_type_cast, x, output_dtype);
}

MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("type_cast", &pyboost_type_cast, "type_cast", pybind11::arg("x"), pybind11::arg("output_dtype"));
}
