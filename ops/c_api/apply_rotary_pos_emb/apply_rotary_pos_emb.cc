...
class CustomApplyRotaryPosEmb : public InternalKernelMod {
 ...
 protected:
  internal_v2::InternalOpPtr CreateKernel(const internal_v2::InputsImmutableInfoList &inputs,
    ...
    internal_v2::ApplyRotaryPosEmbParam param;
    auto cos_format =
    ...
    return internal_v2::CreateApplyRotaryPosEmbOp(inputs, outputs, param,
                                                  internal_v2::kInternalApplyRotaryPosEmbOpName);
  }
};
}  // namespace ms_custom_ops
...
class ApplyRotaryPosEmbRunner : public InternalPyboostRunner {
 ...
 private:
  int32_t cos_format_{COS_FORMAT_HALF_MODE};
};
...
std::vector<ms::Tensor> npu_apply_rotary_pos_emb(const ms::Tensor &query, const ms::Tensor &key, const ms::Tensor &cos,
  ...
  auto runner = std::make_shared<ApplyRotaryPosEmbRunner>(op_name);
...
auto pyboost_apply_rotary_pos_emb(const ms::Tensor &query, const ms::Tensor &key, const ms::Tensor &cos,
                                  const ms::Tensor &sin, const ms::Tensor &position_ids,
                                  std::optional<int64_t> cos_format) {
  return ms::pynative::PyboostRunner::Call<ms_custom_ops::kNumber2>(ms_custom_ops::npu_apply_rotary_pos_emb, query, key,
                                                                    cos, sin, position_ids, cos_format);
}
...
MS_CUSTOM_OPS_EXTENSION_MODULE(m) {
  m.def("apply_rotary_pos_emb", &pyboost_apply_rotary_pos_emb, "ApplyRotaryPosEmb", pybind11::arg("query"),
        ...
        pybind11::arg("cos_format") = ms_custom_ops::COS_FORMAT_HALF_MODE);
}
