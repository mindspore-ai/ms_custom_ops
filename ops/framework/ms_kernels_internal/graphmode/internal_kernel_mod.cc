...
#include "ops/framework/ms_kernels_internal/internal_tiling_cache.h"
...
bool InternalKernelMod::Init(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) {
  ...
  for (size_t i = 0; i < kernel_inputs_index_.size(); i++) {
    ...
    internal_inputs_shape_.emplace_back(internal_v2::ShapeInfo{0});
  }

  for (size_t i = 0; i < kernel_outputs_index_.size(); i++) {
    internal_outputs_addr_.emplace_back(nullptr);
    internal_outputs_shape_.emplace_back(internal_v2::ShapeInfo{0});
  }

  for (size_t i = 0; i < inputs.size(); i++) {
    if (!(std::any_of(kernel_inputs_index_.begin(), kernel_inputs_index_.end(), [=](auto idx) { return idx == i; }))) {
...
void InternalKernelMod::GetOrGenerateTiling(const std::vector<KernelTensor *> &inputs,
  ...
  if (tiling_cache_item == nullptr) {
    ...
    internal_v2::HostRunInfoPtr host_run_info_ptr = nullptr;
    auto status = internal_op_->Tiling(host_addr, &host_run_info_ptr);
    if (status != internal_v2::kInternalOk || host_run_info_ptr == nullptr) {
    ...
    auto tiling_info = std::make_shared<internal_v2::TilingInfo>(device_addr, nullptr);
...
void InternalKernelMod::GetInternalKernel(const std::vector<KernelTensor *> &inputs,
  ...
  if (IsNeedRecreate(inputs, outputs)) {
    internal_v2::InputsImmutableInfoList inputs_ii;
    internal_v2::OutputsImmutableInfoList outputs_ii;
    for (auto i : kernel_inputs_index_) {
      if (i >= inputs.size()) {
        inputs_ii.emplace_back(internal_v2::DataType::kTypeNone, internal_v2::TensorFormat::kFormatND);
        continue;
      }
    ...
    for (auto i : kernel_outputs_index_) {
      if (i >= outputs.size()) {
        outputs_ii.emplace_back(internal_v2::DataType::kTypeNone, internal_v2::TensorFormat::kFormatND);
        continue;
      }
    ...
    if (status != internal_v2::kInternalOk) {
...
int InternalKernelMod::Resize(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &outputs) {
  ...
  for (auto i : kernel_inputs_index_) {
    if (i >= inputs.size()) {
      internal_inputs_shape_[idx++] = std::move(internal_v2::ShapeInfo{});
      continue;
    }
  ...
  for (auto i : kernel_outputs_index_) {
    if (i >= outputs.size()) {
      internal_outputs_shape_[idx++] = std::move(internal_v2::ShapeInfo{});
      continue;
    }
  ...
  if (internal_ret != internal_v2::kInternalOk) {
...
void InternalKernelMod::UpdateAddr(const std::vector<KernelTensor *> &inputs,
  ...
  for (auto i : kernel_inputs_index_) {
    if (i >= inputs.size()) {
      internal_inputs_addr_[idx++] = nullptr;
      continue;
    }

    if (!IsContiguous(inputs[i])) {
      MS_LOG(EXCEPTION) << "Internal op " << kernel_name_ << " does not support noncontiguous input, index: " << i;
    }

    internal_inputs_addr_[idx++] =
      static_cast<void *>(static_cast<int8_t *>(inputs[i]->device_ptr()) + GetStorageOffset(inputs[i]));
  }

  idx = 0;
  for (auto i : kernel_outputs_index_) {
    if (i >= outputs.size()) {
      internal_outputs_addr_[idx++] = nullptr;
      continue;
    }

    if (!IsContiguous(outputs[i])) {
      MS_LOG(EXCEPTION) << "Internal op " << kernel_name_ << " does not support noncontiguous output, index: " << i;
    }
    internal_outputs_addr_[idx++] =
      static_cast<void *>(static_cast<int8_t *>(outputs[i]->device_ptr()) + GetStorageOffset(outputs[i]));
  }
...
bool InternalKernelMod::Launch(const std::vector<KernelTensor *> &inputs, const std::vector<KernelTensor *> &workspace,
  ...
  return (status == internal_v2::InternalStatus::kInternalOk);
}
}  // namespace ms_custom_ops
