...
#include "internal.h"  // NOLINT(build/include_subdir)

namespace ms_custom_ops {
using namespace mindspore::ops;  // NOLINT(build/namespaces)
...
class InternalKernelMod : public KernelMod {
 ...
 protected:
  ...
  // API for subclass to configure NZ format outputs
  void SetNzOutputIndices(const std::vector<size_t> &indices) {
    nz_output_indices_ = indices;
  }

  void AddNzOutputIndex(size_t index) {
    nz_output_indices_.push_back(index);
  }

  void ClearNzOutputIndices() {
    nz_output_indices_.clear();
  }
  ...
  internal_v2::InputsAddrList internal_inputs_addr_;
  internal_v2::OutputsAddrList internal_outputs_addr_;
  internal_v2::WsAddrList internal_wss_addr_;
