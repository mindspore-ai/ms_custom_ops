...
size_t TilingMemPool::GetAlignedSize(size_t size) {
  return (size + block_size_ - 1) & ~(block_size_ - 1);
}

TilingMemPool::TilingMemPool(size_t block_size, size_t block_num)
...
void TilingMemPool::Rearrange() {
  ...
  auto num = static_cast<int64_t>(head_ > tail_ ? tail_ + block_num_ - head_
                                                : tail_ - head_);
  ...
  for (auto i = head_; i < tail_; i++) {
    ...
    if (slot.length_ == aligned_size) {
      if (i == head_) {
        ...
        // the slot is in the middle of head and slot, move the head to this
        // empty slot
...
void TilingMemPool::Free(void *addr, size_t size) {
  ...
  auto offset =
      static_cast<size_t>(static_cast<int8_t *>(addr) - mem_base_ptr_);
  auto aligned_size = GetAlignedSize(size);
  bool merged = false;
  for (auto i = head_; i < tail_; i++) {
    auto &slot = mem_slots_[i];
...
TilingMemPoolDevice::TilingMemPoolDevice(size_t block_size, size_t block_num)
    : TilingMemPool(block_size, block_num) {
  SetName("DEVICE");
}
...
TilingMemPoolHost::TilingMemPoolHost(size_t block_size, size_t block_num)
    : TilingMemPool(block_size, block_num) {
  SetName("HOST");
}
...
void TilingMemMgr::CopyAsync(const void *host_ptr, void *device_ptr, size_t size) {
  ...
  if (default_stream_ == nullptr) {
    auto default_stream_id =
        device_context_->device_res_manager_->DefaultStream();
    default_stream_ =
        device_context_->device_res_manager_->GetStream(default_stream_id);
  }
  auto ret = kernel::InternalAscendAdapter::AscendMemcpyAsync(
