...
class CustomOPCompiler():
    ...
    def copy_code_file(self):
        ...
        def _copy_dir(source_dir, dest_dir):
            for item in os.listdir(source_dir):
                source_item_path = os.path.join(source_dir, item)
                dest_item_path = os.path.join(dest_dir, item)
                shutil.copytree(source_item_path, dest_item_path)

        common_host = os.path.join(self.args.common_dirs, OP_HOST)
        common_kernel = os.path.join(self.args.common_dirs, OP_KERNEL)

        if os.path.exists(common_host):
            logger.info("Copy common files for host: %s", common_host)
            target_path = os.path.join(self.custom_project, OP_HOST)
            _copy_dir(common_host, target_path)

        if os.path.exists(common_kernel):
            logger.info("Copy common files for kernel: %s", common_kernel)
            target_path = os.path.join(self.custom_project, OP_KERNEL)
            _copy_dir(common_kernel, target_path)
        ...
        for op_dir in self.op_dirs:
            ...
            if not os.path.exists(op_host_dir) or not os.path.exists(op_kernel_dir):
                logger.warning(
                    "The %s dose not contain %s or %s, skipped!",
                    op_dir,
                    op_host_dir,
                    op_kernel_dir,
                )
    ...
    def trans_soc_version(self, soc_version_args):
        """Convert soc_version string into the ai_core compute unit format"""
    ...
    def generate_compile_project(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            gen_command = ["msopgen", "gen", "-i", custom_json, "-c",
                           compute_unit, "-lan", "cpp", "-out", self.custom_project]
