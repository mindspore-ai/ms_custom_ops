...
def run_mla(test_param: MlaTestParam):
    ...
    if test_param.is_quant_flag:
        # Reshape to merge last two dimensions for trans_data conversion
        # From (num_blocks, block_size, kv_heads, head_size) to (num_blocks, block_size, kv_heads * head_size)
        ctkv_shape = ctkv_tensor.shape
        ctkv_tensor = ops.reshape(ctkv_tensor, (ctkv_shape[0], ctkv_shape[1], -1))
        k_rope_shape = k_rope_tensor.shape
        k_rope_tensor = ops.reshape(k_rope_tensor, (k_rope_shape[0], k_rope_shape[1], -1))
