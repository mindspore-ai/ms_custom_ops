...
class RotaryEmbedding(nn.Cell):
    ...
    def __init__(self, dim, base=10000, max_seq_len=2048, cos_dtype=np.float32, cos_format=0):
        ...
        # self.apply_rotary_pos_emb = ms_custom_ops.ApplyRotaryPosEmb(cos_format)
        self.dim = dim
        self.cos_format = cos_format

    def construct(self, query, key, position_ids):
        """forward"""
        if self.cos_format in (2, 3):
            batch, seq_len, _ = query.shape
            if seq_len == 1:
                freqs_cos = ops.gather(self.cos, position_ids, 0)
                freqs_sin = ops.gather(self.sin, position_ids, 0)
            else:
                freqs_cos = ops.tile(ops.gather(
                    self.cos, position_ids, 0), (batch, 1))
                freqs_sin = ops.tile(ops.gather(
                    self.sin, position_ids, 0), (batch, 1))
            query_embed, key_embed = ms_custom_ops.apply_rotary_pos_emb(
                query, key, freqs_cos, freqs_sin, position_ids, self.cos_format)
        else:
            query_embed, key_embed = ms_custom_ops.apply_rotary_pos_emb(
    ...
    def cal_truth_numpy(self, query, key, position_ids, query_dtype, cos_format):
        ...
        if cos_format in (1, 3):
            tmp_shape = cos1.shape
            cos1 = cos1.reshape(
                (-1, tmp_shape[-1] // 2, 2)).transpose((0, 2, 1)).reshape(tmp_shape)
            sin1 = sin1.reshape(
                (-1, tmp_shape[-1] // 2, 2)).transpose((0, 2, 1)).reshape(tmp_shape)
...
def run(net, seq_len, batch, num_head_q, num_head_k, hidden_dim, max_seq_len, query_dtype, pos_dtype, ndim=3,
        cos_format=0):
    """run case"""
    if ndim == 3:
        query = np.random.rand(batch, seq_len, num_head_q *
                               hidden_dim).astype(np.float32)
        key = np.random.rand(batch, seq_len, num_head_k *
                             hidden_dim).astype(np.float32)
    else:
        query = np.random.rand(batch, seq_len, num_head_q,
                               hidden_dim).astype(np.float32)
    ...
    if seq_len == 1:
        position_ids = np.random.randint(
            0, max_seq_len, [batch], dtype=pos_dtype)
    else:
        position_ids = np.arange(seq_len).astype(pos_dtype)
    ...
    query1 = query.reshape(
        (batch, seq_len, num_head_q, hidden_dim)).transpose((0, 2, 1, 3))
    key1 = key.reshape((batch, seq_len, num_head_k, hidden_dim)
                       ).transpose((0, 2, 1, 3))
    if cos_format in (1, 3):
        tmp_shape1, tmp_shape2 = query1.shape, key1.shape
        query1 = query1.reshape(-1, hidden_dim // 2,
                                2).transpose((0, 2, 1)).reshape(tmp_shape1)
        key1 = key1.reshape(-1, hidden_dim // 2,
                            2).transpose((0, 2, 1)).reshape(tmp_shape2)
    query_embed2, key_embed2 = net.cal_truth_numpy(
        query1, key1, position_ids, query_dtype, cos_format)
    ...
    if cos_format in (1, 3):
        ...
        query_embed2 = query_embed2.reshape(-1, 2, hidden_dim //
                                            2).transpose((0, 2, 1)).reshape(tmp_shape1)
        key_embed2 = key_embed2.reshape(-1, 2, hidden_dim //
                                        2).transpose((0, 2, 1)).reshape(tmp_shape2)
    np.testing.assert_allclose(
        query_embed1, query_embed2, rtol=1e-2, atol=1e-2)
...
@pytest.mark.parametrize('cos_format', [2, 3])
...
def test_rope_float16(query_dtype, cos_dtype, cos_format, batch_size, seq_len, num_head, exec_mode):
    """
    ...
    run(net, seq_len, batch_size, num_head, num_head, hidden_dim,
        max_seq_len, query_dtype, np.int32, ndim, cos_format)
...
@pytest.mark.parametrize('num_head', [32])
@pytest.mark.parametrize('exec_mode', [context.GRAPH_MODE, context.PYNATIVE_MODE])
def test_rope_bfloat16(query_dtype, cos_dtype, cos_format, batch_size, seq_len, num_head, exec_mode):
    """
    Feature:apply_rotary_pos_emb kernel.
    Description: test for apply_rotary_pos_emb ops.
    Expectation:should pass for all testcases.
