...
# ============================================================================
"""test apply_rotary_pos_emb_ms cases"""
import time
...
import numpy as np
import pytest
from mindspore import Profiler, Tensor, context, mint, nn
...
def golden_apply_rotary_emb(x: Tensor, cos: Tensor, sin: Tensor, is_neox_style: bool) -> Tensor:
    """
    Args:
...
def golden_apply_rotary_emb_split(net, query_dtype, layout, rotary_mode, tokens, head_num_q, head_num_k, head_dim,
                                  rotary_dim, is_profiler=False):
    """golden_apply_rotary_emb_split"""
    ...
    if not is_profiler:
        ...
        np.testing.assert_allclose(golden_q.asnumpy(
        ), out_query.asnumpy(), rtol=1e-4, atol=1e-4, err_msg=" query ")
        np.testing.assert_allclose(
            golden_k.asnumpy(), out_key.asnumpy(), rtol=1e-4, atol=1e-4, err_msg=" key ")
    else:
...
class ApplyRotaryEmbMSNet(nn.Cell):
    """Reshape and cache operation for NZ/ND format with all parameters"""

    @jit_for_graph_mode
    def construct(self, query, key, cos, sin, layout, rotary_mode):
        return ms_custom_ops.apply_rotary_pos_emb_ms(query, key, cos, sin, layout, rotary_mode)


def run_rope_interleave(net, query_dtype, layout, rotary_mode, tokens, head_num_q, head_num_k, head_dim,
                        is_profiler=False):
    """rotary_dim same as head_size"""
    ...
    golden_k = golden_apply_rotary_emb(key, cos, sin, False)
    if not is_profiler:
        net(query, key, cos, sin, layout, rotary_mode)
        np.testing.assert_allclose(
            golden_q.asnumpy(), query.asnumpy(), rtol=1e-4, atol=1e-4, err_msg=" query ")
        np.testing.assert_allclose(
            golden_k.asnumpy(), key.asnumpy(), rtol=1e-4, atol=1e-4, err_msg=" key ")
    else:
        profiler = Profiler(profiler_level=ProfilerLevel.Level2,
                            activities=[ProfilerActivity.CPU,
                                        ProfilerActivity.NPU],
...
def run_rope_interleave_split(net, query_dtype, layout, rotary_mode, tokens, head_num_q, head_num_k, head_dim,
                              rotary_dim, is_profiler=False):
    """head_size greater rotary_dim"""
    cos_head_dim = rotary_dim // 2
    np_query = np.random.random((tokens, head_num_q, head_dim))
    np_key = np.random.random((tokens, head_num_k, head_dim))
    np_cos = np.random.random((tokens, cos_head_dim))
...
def test_apply_rotary_pos_emb_ms_interleave(exec_mode, query_dtype, layout, rotary_mode, tokens, head_num_q, head_num_k, head_dim):
    """
    Feature: aclnnApplyRotaryPosEmb kernel.
    Description: test for ApplyRotaryPosEmb ops.
    ...
    net = ApplyRotaryEmbMSNet()
    run_rope_interleave(net, query_dtype, layout, rotary_mode,
                        tokens, head_num_q, head_num_k, head_dim)
...
def test_apply_rotary_pos_emb_ms_interleave_split(exec_mode, query_dtype, layout, rotary_mode, tokens, head_num_q, head_num_k,
                                  head_dim, rotary_dim):
    """
    Feature: aclnnApplyRotaryPosEmb kernel.
    Description: test for ApplyRotaryPosEmb ops.
    ...
    ms.set_context(jit_config={"jit_level": "O0", "infer_boost": "on"})
    net = ApplyRotaryEmbMSNet()
    run_rope_interleave_split(net, query_dtype, layout, rotary_mode, tokens, head_num_q, head_num_k, head_dim,
                              rotary_dim)
