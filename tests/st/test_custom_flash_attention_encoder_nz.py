...
class FlashAttentionEncoderNzNet(nn.Cell):
    ...
    def __init__(self, heads: int, scale_value: float, kv_heads: int, mask_type: int,
                 kernel_type: int, window_size: int, cache_type: int, input_format: int = 1):
        super().__init__()
        self.heads = heads
    ...
    def construct(self, q_nd, k_nd, v_nd, mask_nd, alibi_slopes, q_seq_lens, kv_seq_lens):
        # Convert ND to NZ format within graph
        ...
        out_nz = ms_custom_ops.flash_attention_encoder(
            ...
            self.kernel_type, self.window_size, self.cache_type, self.input_format)
...
def _run_nz_test(test_case: TestCase, run_mode: int, dynamic: bool = False):
    ...
    # Build ALIBI slopes if needed
    alibi_slopes = (
        FlashAttentionTestSuite._get_alibi_slopes(test_case.heads)
    ...
    net = FlashAttentionEncoderNzNet(
        ...
        test_case.kernel_type, test_case.window_size, test_case.cache_type, input_format=1
    )
    ...
    assert test_suite.validate_output(out_np, golden.astype(np.float32), test_case.np_dtype, test_case.heads, int(test_case.q_seq.max()))
...
def test_flash_attention_encoder_th_nz_norm_mask(run_mode, dynamic, np_dtype):
    ...
    Description: Trans_data (ND->NZ) within Net graph for Q/K/V/Mask, input_format=1, run across modes and dynamic/static shapes
    Expectation: Operator executes successfully and output matches golden attention within tolerance
    """
