...
class ReshapeAndCacheNpdAll(nn.Cell):
    ...
    def __init__(self, cache_layout="BNSD", kv_layout="NPD"):
        super().__init__()
    ...
    def construct(
        ...
        k_out, value_out = ms_custom_ops.reshape_and_cache_npd(
            ...
            block_tbl=block_tbl,
            kv_cache_layout=self.cache_layout,
            key_value_layout=self.kv_layout,
        )
...
def create_ms_inputs(
    np_k, np_v, np_k_cache, np_v_cache, np_slot_map, q_seq, kv_seq, block_tbl
):
...
class TestDataGenerator:
    ...
    def create_random_data(
        ...
        if dtype == np.int8:
            return np.random.randint(
                low=-128, high=127, size=shape, dtype=np.int8
            )
    ...
    def create_blk_tbl(page_num: int, batch: int) -> np.ndarray:
        return np.arange(page_num, dtype=np.int32).reshape(
            batch, (page_num + batch - 1) // batch
        )

    @staticmethod
    def get_update_shapes(
        kv_dim: int, k_head_dim=None, v_head_dim=None, batch=None, head_num=None
        ...
        if kv_dim == 2:
            key_update_shape = (
                ...
                actual_head_num * actual_k_head_dim,
            )
            value_update_shape = (
                actual_batch * SEQ_LEN,
                actual_head_num * actual_v_head_dim,
            )
        ...
        page_num = ((SEQ_LEN + SLOT_SIZE - 1) // SLOT_SIZE) * actual_batch
        return key_update_shape, value_update_shape, num_tokens, page_num

    @staticmethod
    def get_update_shape(
        kv_dim: int,
        is_key: bool = True,
        k_head_dim=None,
        v_head_dim=None,
        ...
        key_shape, value_shape, num_tokens, page_num = (
            TestDataGenerator.get_update_shapes(
                kv_dim, k_head_dim, v_head_dim, batch
            )
        )
...
class DataGenerator(TestDataGenerator):
    ...
    def create_inputs(
        ...
        key_new_shape, value_new_shape, num_tokens, page_num = (
            TestDataGenerator.get_update_shapes(
                kv_dim, k_head_dim, v_head_dim, batch, head_num
            )
        )
        key_cache_shape = (
            page_num,
...
def create_inputs(
    ...
    """Legacy function for backward compatibility"""
    return DataGenerator.create_inputs(
        dtype, kv_dim, k_head_dim, v_head_dim, batch, head_num, npd
    )
...
class InferenceEngine:
    ...
    @staticmethod
    def npd_permute(
        np_arr: np.ndarray, heads: int, embed: int, batch: int, ps: int
    ):
        """npd permute function"""
        if np_arr.ndim == 2:
        ...
        assert hd == heads * embed, (
            "Shape mismatch: second dim must be heads*embed"
        )
        assert t_total % batch == 0, (
            "Total tokens must be divisible by batch size"
        )
        ...
        np_arr = np_arr.reshape(batch * blocks, ps, heads, embed).transpose(
            0, 2, 1, 3
        )
    ...
    def npd_kv_inference(
        ...
        return (
            InferenceEngine.npd_permute(
                key, key_head, key_head_dim, batch, slot_size
            ),
            InferenceEngine.npd_permute(
                value, value_head, value_head_dim, batch, slot_size
            ),
        )

    @staticmethod
    def nd_cache_inference(
        key: np.ndarray,
        value: np.ndarray,
        key_cache: np.ndarray,
        value_cache: np.ndarray,
...
def nd_kv_inference(key, value):
    """Legacy function for backward compatibility"""
    return InferenceEngine.nd_kv_inference(key, value)


def nd_cache_inference(key, value, key_cache, value_cache, slot_map):
    """Legacy function for backward compatibility"""
    return InferenceEngine.nd_cache_inference(
        key, value, key_cache, value_cache, slot_map
    )


def npd_cache_inference(key, value, key_cache, value_cache, slot_map):
    return InferenceEngine.npd_cache_inference(
        key, value, key_cache, value_cache, slot_map
    )


def npd_kv_inference(
    key, value, key_cache, value_cache, block_tbl, cache_layout
):
    return InferenceEngine.npd_kv_inference(
        key, value, key_cache, value_cache, block_tbl, cache_layout
    )
...
@pytest.mark.parametrize(
    "run_mode", [context.GRAPH_MODE, context.PYNATIVE_MODE]
)
...
def test_reshape_and_cache_kv_cache_bsnd_kv_th(
    np_dtype, kv_embed, batch, head_num, run_mode
):
    """
    ...
    bnsd = kv_cache_mode == "BNSD"

    net = ReshapeAndCacheNpdAll(kv_cache_mode, kv_mode)
    (
        np_k,
        np_v,
        np_k_cache,
        ...
        np_dtype, kv_dim, kv_embed, kv_embed, batch, head_num, bnsd
    )

    if kv_mode == "NPD":
        np_k_out, np_v_out = npd_kv_inference(
            np_k, np_v, np_k_cache, np_v_cache, np_block_tbl, bnsd
        )
    else:
        np_k_out, np_v_out = nd_kv_inference(np_k, np_v)

    if bnsd:
        np_k_cache_out, np_v_cache_out = npd_cache_inference(
            np_k, np_v, np_k_cache, np_v_cache, np_slot_map
        )
    else:
        np_k_cache_out, np_v_cache_out = nd_cache_inference(
            np_k, np_v, np_k_cache, np_v_cache, np_slot_map
        )
...
@pytest.mark.parametrize("batch", [1, 2, 8, 13, 16])
@pytest.mark.parametrize("head_num", [4, 8, 32])
def test_reshape_and_cache_kv_cache_bsnd_kv_npd(
    np_dtype, kv_embed, batch, head_num, run_mode
):
    """
    Feature: Test ReshapeAndCacheNpd.
...
@pytest.mark.parametrize("batch", [1, 2, 8, 13, 16])
@pytest.mark.parametrize("head_num", [4, 8, 32])
def test_reshape_and_cache_kv_cache_bnsd_kv_th(
    np_dtype, kv_embed, batch, head_num, run_mode
):
    """
    Feature: Test ReshapeAndCacheNpd.
...
@pytest.mark.parametrize("batch", [1, 2, 8, 13, 16])
@pytest.mark.parametrize("head_num", [4, 8, 32])
def test_reshape_and_cache_kv_cache_bnsd_kv_npd(
    np_dtype, kv_embed, batch, head_num, run_mode
):
    """
    Feature: Test ReshapeAndCacheNpd.
