...
class ReshapeAndCacheNpdAll(nn.Cell):
    ...
    def __init__(self, cache_layout="NPD", kv_layout="NPD"):
        super().__init__()
...
class TestDataGenerator:
    ...
    def get_update_shape(
        kv_dim: int, is_key: bool = True, k_head_dim=None, v_head_dim=None, batch=None
    ) -> Tuple[Tuple[int, ...], int, int]:
        """Legacy method for backward compatibility"""
        key_shape, value_shape, num_tokens, page_num = TestDataGenerator.get_update_shapes(
...
class DataGenerator(TestDataGenerator):
    ...
    @staticmethod
    def create_inputs(
        dtype: np.dtype, kv_dim: int, k_head_dim=None, v_head_dim=None, batch=None, head_num=None, npd=False
    ) -> Tuple[np.ndarray, ...]:
        ...
        key_new_shape, value_new_shape, num_tokens, page_num = TestDataGenerator.get_update_shapes(
            kv_dim, k_head_dim, v_head_dim, batch, head_num
        )
        key_cache_shape = (page_num, SLOT_SIZE, actual_head_num, actual_k_head_dim)
        value_cache_shape = (page_num, SLOT_SIZE, actual_head_num, actual_v_head_dim)
        if npd:
...
def create_inputs(dtype=np.float16, kv_dim=3, k_head_dim=None, v_head_dim=None, batch=None, head_num=None, npd=False):
    """Legacy function for backward compatibility"""
    return DataGenerator.create_inputs(dtype, kv_dim, k_head_dim, v_head_dim, batch, head_num, npd)
...
def nd_cache_inference(key, value, key_cache, value_cache, slot_map):
    """Legacy function for backward compatibility"""
    return InferenceEngine.nd_cache_inference(key, value, key_cache, value_cache, slot_map)


def npd_cache_inference(key, value, key_cache, value_cache, slot_map):
    return InferenceEngine.npd_cache_inference(key, value, key_cache, value_cache, slot_map)


def npd_kv_inference(key, value, key_cache, value_cache, block_tbl, cache_layout):
    return InferenceEngine.npd_kv_inference(key, value, key_cache, value_cache, block_tbl, cache_layout)
...
def test_reshape_and_cache_npd_kv_npd_ckv(np_dtype, kv_embed, batch, head_num, run_mode):
    """
    ...
    np_k, np_v, np_k_cache, np_v_cache, np_slot_map, np_q_seq, np_kv_seq, np_block_tbl = create_inputs(
        np_dtype, kv_dim, kv_embed, kv_embed, batch, head_num, npd = "NPD"
    )

    if kv_mode == "NPD":
        np_k_out, np_v_out = npd_kv_inference(
            np_k, np_v, np_k_cache, np_v_cache, np_block_tbl, cache_layout= "NPD"
        )
    else:
        np_k_out, np_v_out = nd_kv_inference(
            np_k, np_v
        )

    if kv_cache_mode == "NPD":
...
@pytest.mark.parametrize("batch", [1, 2, 8, 13, 16])
@pytest.mark.parametrize("head_num", [4, 8, 32])
def test_reshape_and_cache_npd_kv_nd_ckv(np_dtype, kv_embed, batch, head_num, run_mode):
    """
    Feature: Test ReshapeAndCacheNpd.
    Description: Test ND format with key and value.
    Expectation: Assert that results are consistent with numpy.
    ...
    np_k, np_v, np_k_cache, np_v_cache, np_slot_map, np_q_seq, np_kv_seq, np_block_tbl = create_inputs(
        np_dtype, kv_dim, kv_embed, kv_embed, batch, head_num, kv_cache_mode == "NPD"
    )

    if kv_mode == "NPD":
        np_k_out, np_v_out = npd_kv_inference(
            np_k, np_v, np_k_cache, np_v_cache, np_block_tbl, kv_cache_mode == "NPD"
        )
    else:
        np_k_out, np_v_out = nd_kv_inference(
            np_k, np_v
...
@pytest.mark.parametrize("batch", [1, 2, 8, 13, 16])
@pytest.mark.parametrize("head_num", [4, 8, 32])
def test_reshape_and_cache_th_kv_npd_ckv(np_dtype, kv_embed, batch, head_num, run_mode):
    """
    Feature: Test ReshapeAndCacheNpd.
    Description: Test ND format with key and value.
    Expectation: Assert that results are consistent with numpy.
...
@pytest.mark.parametrize("batch", [1, 2, 8, 13, 16])
@pytest.mark.parametrize("head_num", [4, 8, 32])
def test_reshape_and_cache_th_kv_nd_ckv(np_dtype, kv_embed, batch, head_num, run_mode):
    """
    Feature: Test ReshapeAndCacheNpd.
    Description: Test ND format with key and value.
    Expectation: Assert that results are consistent with numpy.
