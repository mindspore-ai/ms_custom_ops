...
# ============================================================================
"""apply_rotary_pos_emb_cops test case"""
...
def apply_rotary_pos_emb_cops(query, key, cos, sin, layout, rotary_mode="half"):
    """
    自定义算子：应用旋转位置编码V2版本
    参数:
...
def apply_rotary_pos_emb_quarter(query, key, cos, sin):
    ...
    query_rotate = np.concatenate(
        (-query_q2, query_q1, -query_q4, query_q3), axis=-1)
    q_embed = query * cos + query_rotate * sin
...
def apply_rotary_pos_emb_interleave(query, key, cos, sin):
    ...
    query_rotate_flat = np.concatenate(
        (-query_q2_flat, query_q1_flat), axis=-1)
...
class ApplyRotaryPosEmbNet(ms.nn.Cell):
    ...
    def construct(self, query, key, cos, sin, layout, rotary_mode):
        return ms_custom_ops.apply_rotary_pos_emb_cops(
            query, key, cos, sin, layout, rotary_mode
        )


def run(net, cos_dtype, seq_len, batch_size, num_head, hidden_dim, query_dtype, rotary_mode="half"):
    """run case"""
    ...
    golden_query_emb, golden_key_emb = apply_rotary_pos_emb_cops(
    ...
    cos2 = Tensor(cos2, dtype=get_ms_dtype(cos_dtype))
    sin2 = Tensor(sin2, dtype=get_ms_dtype(cos_dtype))

    net(query2, key2, cos2, sin2, "BSND", rotary_mode)
    np.testing.assert_allclose(golden_query_emb, query2, rtol=1e-2, atol=1e-2)
    np.testing.assert_allclose(golden_key_emb, key2, rtol=1e-2, atol=1e-2)
...
def test_rope_float16(exec_mode, query_dtype, cos_dtype, rotary_mode, batch_size, seq_len, num_head):
    """
    Feature:aclnnApplyRotaryPosEmb kernel.
    Description: test for ApplyRotaryPosEmbCops ops.
    ...
    run(net, cos_dtype, seq_len, batch_size, num_head,
        hidden_dim, query_dtype, rotary_mode)
