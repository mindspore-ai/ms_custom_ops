...
class TestDataGenerator:
    ...
    @staticmethod
    def get_update_shapes(kv_dim: int, k_head_dim=None, v_head_dim=None) -> Tuple[Tuple[int, ...], Tuple[int, ...],
                                                                                  int]:
        ...
        if kv_dim == 2:
            key_update_shape = (BATCH_SIZE * SEQ_LEN,
                                NUM_HEADS * actual_k_head_dim)
            value_update_shape = (BATCH_SIZE * SEQ_LEN,
                                  NUM_HEADS * actual_v_head_dim)
    ...
    @staticmethod
    def get_update_shape(kv_dim: int, is_key: bool = True, k_head_dim=None, v_head_dim=None) -> Tuple[Tuple[int, ...],
                                                                                                      int]:
        """Legacy method for backward compatibility"""
        key_shape, value_shape, num_tokens = TestDataGenerator.get_update_shapes(
            kv_dim, k_head_dim, v_head_dim)
...
class NDDataGenerator(TestDataGenerator):
    ...
    def create_inputs(dtype: np.dtype, kv_dim: int, k_head_dim=None, v_head_dim=None) -> Tuple[np.ndarray, ...]:
        ...
        value_cache_shape = (NUM_SLOTS, SLOT_SIZE,
                             NUM_HEADS, actual_v_head_dim)
        key_update_shape, value_update_shape, num_tokens = TestDataGenerator.get_update_shapes(
            kv_dim, k_head_dim, v_head_dim)

        key_update = TestDataGenerator.create_random_data(
            key_update_shape, dtype)
        value_update = TestDataGenerator.create_random_data(
            value_update_shape, dtype)
        key_cache = TestDataGenerator.create_random_data(
            key_cache_shape, dtype)
        value_cache = TestDataGenerator.create_random_data(
            value_cache_shape, dtype)
...
def test_reshape_and_cache_nd_key_value(np_dtype, kv_dim, run_mode):
    ...
    # Run test
    net(ms_k, ms_v, ms_k_cache, ms_v_cache, ms_slot_map, 0)
...
def test_reshape_and_cache_nd_key(np_dtype, kv_dim, run_mode):
    ...
    # Run test
    net(ms_k, key_cache=ms_k_cache, slot_map=ms_slot_map, cache_mode=0)
    TestResultVerifier.verify_results(ms_k_cache, np_k_cache_out, np_dtype)
...
def test_reshape_and_cache_nz_different_key_value_dimensions(kv_dim, run_mode):
    ...
    def run_test():
        ...
        test_config = TestConfig(
            device_target="Ascend", mode=run_mode, jit_config=jit_config)
        test_config.apply()
        ...
        assert np_k_cache.shape != np_v_cache.shape, f"Key and value cache should have different shapes:\
              {np_k_cache.shape} vs {np_v_cache.shape}"
        ...
        ms_k_cache = ms_custom_ops.trans_data(
            ms_k_cache, transdata_type=1)  # ND_TO_FRACTAL_NZ
        ms_v_cache = ms_custom_ops.trans_data(
            ms_v_cache, transdata_type=1)  # ND_TO_FRACTAL_NZ

        net(ms_k, ms_v, ms_k_cache, ms_v_cache,
            ms_slot_map, cache_mode=1, head_num=NUM_HEADS)
        ...
        golden_k_output = get_nd_cached_slots(
            np_k_cache_out, np_slot_map)  # Golden is already ND format

        ms_v_output = get_nz_cached_slots(ms_v_cache_np, np_slot_map)
        golden_v_output = get_nd_cached_slots(
            np_v_cache_out, np_slot_map)  # Golden is already ND format
...
def test_reshape_and_cache_different_key_value_dimensions(kv_dim, run_mode):
    ...
    def run_test():
        ...
        # Run test
        net(ms_k, ms_v, ms_k_cache, ms_v_cache, ms_slot_map, cache_mode=0)
        TestResultVerifier.verify_results(
            ms_k_cache, np_k_cache_out, np.float16)
        TestResultVerifier.verify_results(
            ms_v_cache, np_v_cache_out, np.float16)
...
def convert_cache_nz_to_nd_and_verify(ms_k_cache, ms_v_cache, np_k_cache_out, np_v_cache_out,
    ...
    ms_k_cache_nd = ms_custom_ops.trans_data(
        ms_k_cache, transdata_type=0)  # FRACTAL_NZ_TO_ND
    ms_v_cache_nd = ms_custom_ops.trans_data(
        ms_v_cache, transdata_type=0)  # FRACTAL_NZ_TO_ND
...
class NZDataGenerator(TestDataGenerator):
    ...
    @staticmethod
    def create_inputs(k_dtype: np.dtype, v_dtype: np.dtype, kv_dim: int, k_head_dim=None, v_head_dim=None) \
            -> Tuple[np.ndarray, ...]:
        ...
        key_update = TestDataGenerator.create_random_data(
            key_update_shape, k_dtype)
        value_update = TestDataGenerator.create_random_data(
            value_update_shape, v_dtype)
...
def get_nz_cached_slots(cache_nz, slot_map):
    """get_nz_cached_slots"""
    ans = []
    num_slots = cache_nz.shape[0]
    slot_size = cache_nz.shape[1]
    hidden_size = cache_nz.shape[2]

    if cache_nz.dtype == np.int8:
        cache_shape = (num_slots, hidden_size // 32, slot_size, 32)
    else:
        cache_shape = (num_slots, hidden_size // 16, slot_size, 16)
    cache_nz = cache_nz.reshape(cache_shape)
    for _, slot in enumerate(slot_map):
        if slot < 0:
            continue
        slot_idx = slot // slot_size
        slot_offset = slot % slot_size
        tmp = []  # Reset tmp for each slot
        for j in range(cache_nz.shape[1]):
            tmp.append(cache_nz[slot_idx][j][slot_offset])
        ans.append(np.concatenate(tmp, axis=0))
    ans = np.concatenate(ans)
    return ans


def get_nd_cached_slots(cache_nd, slot_map):
    ans = []
    for slot in slot_map:
        ...
        slot_idx = slot // SLOT_SIZE
        slot_offset = slot % SLOT_SIZE
        ans.append(cache_nd[slot_idx][slot_offset])
    ans = np.concatenate(ans)
    return ans
...
def test_reshape_and_cache_nz(k_dtype, v_dtype, kv_dim, run_mode):
    ...
    # Setup context
    jit_config = {"jit_level": "O0"}
    test_config = TestConfig(device_target="Ascend",
                             mode=run_mode, jit_config=jit_config)
    test_config.apply()
