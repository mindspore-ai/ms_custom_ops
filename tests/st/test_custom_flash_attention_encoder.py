...
"""Tests for the `flash_attention_encoder` custom operator on Ascend devices.

This module defines a unified test suite that validates functional correctness
and numerical precision of the encoder-side flash attention implementation
under different layouts, data types, mask types, and execution modes.
"""
...
class FlashAttentionTestSuite:
    ...
    def _get_pre_mask_coef(np_dtype: np.dtype, is_alibi: bool = False) -> float:
        ...
        if np_dtype == np.float16:
            return -10000.0
        if np_dtype == np_bfloat16 and is_alibi:
            return -float("inf")
        if np_dtype == np.float32 and is_alibi:
            return 1.0
        # bf16 non-alibi
        return 1.0
    ...
    def build_operator_mask(self, mask_type: int, max_seq: int, np_dtype: np.dtype, batch: int,
        ...
        if mask_type == MASK_NORM_COMPRESS:
            ...
            return result_mask
        if mask_type in (MASK_SWA_NORM, MASK_SWA_COMPRESS):
    ...
    def build_golden_mask(self, mask_type: int, ql: int, kl: int, np_dtype: np.dtype,
        ...
        if mask_type == MASK_SWA_COMPRESS:
    ...
    def compute_golden_attention(self, q: np.ndarray, k: np.ndarray, v: np.ndarray, q_seq: np.ndarray,
        ...
        for b, (ql, kl) in enumerate(zip(q_seq, kv_seq)):
            ql, kl = int(ql), int(kl)
            if ql == 0:
                continue
    ...
    def run_test_case(self, test_case, dynamic: bool = False) -> None:
        ...
        alibi_needed = (
            test_case.mask_type == MASK_ALIBI or test_case.use_alibi
        )
        if alibi_needed:
            alibi_slopes = self.get_alibi_slopes(test_case.heads)
        else:
            alibi_slopes = None
