...
def _init_env():
    ...
    internal_lib_path = os.path.join(
        current_path,
        "prebuild/ms_kernels_internal/internal_kernel/lib/libms_kernels_internal_v2.so",
    )

    if not os.path.exists(internal_lib_path):
        raise FileNotFoundError(
            f"MS internal kernel library not found at {internal_lib_path}"
        )
